//===- MaxisAsmPrinter.h - Maxis LLVM Assembly Printer -----------*- C++ -*--===//
//
...
//===----------------------------------------------------------------------===//
//
// Maxis Assembly printer class.
//
//===----------------------------------------------------------------------===//

#ifndef LLVM_LIB_TARGET_MAXIS_MAXISASMPRINTER_H
...
class MaxisTargetStreamer;
class Module;
class raw_ostream;
class TargetMachine;

class LLVM_LIBRARY_VISIBILITY MaxisAsmPrinter : public AsmPrinter {
  MaxisTargetStreamer &getTargetStreamer() const;
...
public:
  // XRay-specific lowering for Maxis.
...
private:
  ...
  std::map<const char *, const Maxis16HardFloatInfo::FuncSignature *>
      StubsNeeded;
  ...
  void EmitSwapFPIntParams(const MCSubtargetInfo &STI,
                           Maxis16HardFloatInfo::FPParamVariant, bool LE,
                           bool ToFP);

  void EmitSwapFPIntRetval(const MCSubtargetInfo &STI,
                           Maxis16HardFloatInfo::FPReturnVariant, bool LE);

  void EmitFPCallStub(const char *, const Maxis16HardFloatInfo::FuncSignature *);
...
public:
  ...
  const MaxisFunctionInfo *MaxisFI;
  MaxisMCInstLower MCInstLowering;

  explicit MaxisAsmPrinter(TargetMachine &TM,
                          std::unique_ptr<MCStreamer> Streamer)
      : AsmPrinter(TM, std::move(Streamer)), MCInstLowering(*this) {}

  StringRef getPassName() const override { return "Maxis Assembly Printer"; }

  bool runOnMachineFunction(MachineFunction &MF) override;

  void EmitConstantPool() override {
    bool UsingConstantPools =
      (Subtarget->inMaxis16Mode() && Subtarget->useConstantIslands());
...
#endif // LLVM_LIB_TARGET_MAXIS_MAXISASMPRINTER_H
