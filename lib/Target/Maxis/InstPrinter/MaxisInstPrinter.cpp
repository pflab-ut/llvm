//===-- MaxisInstPrinter.cpp - Convert Maxis MCInst to assembly syntax ------===//
//
...
//===----------------------------------------------------------------------===//
//
// This class prints an Maxis MCInst to a .s file.
//
//===----------------------------------------------------------------------===//

#include "MaxisInstPrinter.h"
...
const char* Maxis::MaxisFCCToString(Maxis::CondCode CC) {
  switch (CC) {
  case FCOND_F:
...
void MaxisInstPrinter::printRegName(raw_ostream &OS, unsigned RegNo) const {
  OS << '$' << StringRef(getRegisterName(RegNo)).lower();
}

void MaxisInstPrinter::printInst(const MCInst *MI, raw_ostream &O,
  ...
  case Maxis::SaveX16:
    ...
    O << "\n";
    return;
  case Maxis::Restore16:
    O << "\trestore\t";
    printSaveRestore(MI, O);
    O << " # 16 bit inst\n";
    return;
  case Maxis::RestoreX16:
    O << "\trestore\t";
    printSaveRestore(MI, O);
    O << "\n";
    return;
...
void MaxisInstPrinter::printOperand(const MCInst *MI, unsigned OpNo,
                                   raw_ostream &O) {
...
void MaxisInstPrinter::printUImm(const MCInst *MI, int opNum, raw_ostream &O) {
...
printFCCOperand(const MCInst *MI, int opNum, raw_ostream &O) {
  const MCOperand& MO = MI->getOperand(opNum);
  O << MaxisFCCToString((Maxis::CondCode)MO.getImm());
}
...
bool MaxisInstPrinter::printAlias(const char *Str, const MCInst &MI,
...
bool MaxisInstPrinter::printAlias(const MCInst &MI, raw_ostream &OS) {
  ...
  case Maxis::BGEZAL:
    ...
    return isReg<Maxis::ZERO>(MI, 0) && printAlias("bal", MI, 1, OS);
  case Maxis::BC1T:
    // bc1t $fcc0, $L1 => bc1t $L1
    return isReg<Maxis::FCC0>(MI, 0) && printAlias("bc1t", MI, 1, OS);
  case Maxis::BC1F:
...
void MaxisInstPrinter::printSaveRestore(const MCInst *MI, raw_ostream &O) {
