//===- Maxis16HardFloat.cpp for Maxis16 Hard Float --------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines a pass needed for Maxis16 Hard Float
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "maxis16-hard-float"

namespace {

  class Maxis16HardFloat : public ModulePass {
  public:
    static char ID;

    Maxis16HardFloat() : ModulePass(ID) {}

    StringRef getPassName() const override { return "MAXIS16 Hard Float Pass"; }
...
// We swap between FP and Integer registers to allow Maxis16 and Maxis32 to
// interoperate
...
static void assureFPCallStub(Function &F, Module *M,
  ...
  std::string SectionName = ".maxis16.call.fp." + Name;
    ...
    for (auto &I: BB) {
      if (const ReturnInst *RI = dyn_cast<ReturnInst>(&I)) {
        ...
        static const char *const Helper[NoFPRet] = {
          "__maxis16_ret_sf", "__maxis16_ret_df", "__maxis16_ret_sc",
          "__maxis16_ret_dc"
        };
        ...
        // this __Maxis16RetHelper indicates that so that later
...
static void createFPFnStub(Function *F, Module *M, FPParamVariant PV,
  ...
  LLVMContext &Context = M->getContext();
  std::string Name = F->getName();
  std::string SectionName = ".maxis16.fn." + Name;
...
// we are compiling as maxis16.
// For all maxis16 functions (that are not stubs we have already generated), or
// declared via attributes as nomaxis16, we must:
//    1) fixup all returns of float, double, single and double complex
//       by calling a helper function before the actual return.
//    2) generate helper functions (stubs) that can be called by maxis32
...
//       maxis16 functions can call extern functions of unknown type (maxis16 or
//       maxis32).
...
bool Maxis16HardFloat::runOnModule(Module &M) {
  auto &TM = static_cast<const MaxisTargetMachine &>(
      getAnalysis<TargetPassConfig>().getTM<TargetMachine>());
  DEBUG(errs() << "Run on Module Maxis16HardFloat\n");
  bool Modified = false;
  for (Module::iterator F = M.begin(), E = M.end(); F != E; ++F) {
    if (F->hasFnAttribute("nomaxis16") &&
        F->hasFnAttribute("use-soft-float")) {
      removeUseSoftFloat(*F);
      continue;
    }
    if (F->isDeclaration() || F->hasFnAttribute("maxis16_fp_stub") ||
        F->hasFnAttribute("nomaxis16")) continue;
...
ModulePass *llvm::createMaxis16HardFloatPass() {
  return new Maxis16HardFloat();
}
