//===- MaxisSEISelLowering.cpp - MaxisSE DAG Lowering Interface -------------===//
//
...
//===----------------------------------------------------------------------===//
//
// Subclass of MaxisTargetLowering specialized for maxis32/64.
//
//===----------------------------------------------------------------------===//

#include "MaxisSEISelLowering.h"
...
MaxisSETargetLowering::MaxisSETargetLowering(const MaxisTargetMachine &TM,
                                           const MaxisSubtarget &STI)
    : MaxisTargetLowering(TM, STI) {
  // Set up the register classes
  addRegisterClass(MVT::i32, &Maxis::GPR32RegClass);

  if (Subtarget.isGP64bit())
    addRegisterClass(MVT::i64, &Maxis::GPR64RegClass);
  ...
  if (Subtarget.hasDSP()) {
    ...
    for (unsigned i = 0; i < array_lengthof(VecTys); ++i) {
      addRegisterClass(VecTys[i], &Maxis::DSPRRegClass);
  ...
  if (Subtarget.hasMSA()) {
    addMSAIntType(MVT::v16i8, &Maxis::MSA128BRegClass);
    addMSAIntType(MVT::v8i16, &Maxis::MSA128HRegClass);
    addMSAIntType(MVT::v4i32, &Maxis::MSA128WRegClass);
    addMSAIntType(MVT::v2i64, &Maxis::MSA128DRegClass);
    addMSAFloatType(MVT::v8f16, &Maxis::MSA128HRegClass);
    addMSAFloatType(MVT::v4f32, &Maxis::MSA128WRegClass);
    addMSAFloatType(MVT::v2f64, &Maxis::MSA128DRegClass);

    // f16 is a storage-only type, always promote it to f32.
    addRegisterClass(MVT::f16, &Maxis::MSA128HRegClass);
  ...
  if (!Subtarget.useSoftFloat()) {
    addRegisterClass(MVT::f32, &Maxis::FGR32RegClass);

    // When dealing with single precision only, use libcalls
    if (!Subtarget.isSingleFloat()) {
      if (Subtarget.isFP64bit())
        addRegisterClass(MVT::f64, &Maxis::FGR64RegClass);
      else
        addRegisterClass(MVT::f64, &Maxis::AFGR64RegClass);
    }
  }
  ...
  if (Subtarget.hasMaxis32r6()) {
    ...
    assert(Subtarget.isFP64bit() && "FR=1 is required for MAXIS32r6");
  ...
  if (Subtarget.hasMaxis64r6()) {
    // MAXIS64r6 replaces the accumulator-based multiplies with a three register
    // instruction
...
llvm::createMaxisSETargetLowering(const MaxisTargetMachine &TM,
                                 const MaxisSubtarget &STI) {
  return new MaxisSETargetLowering(TM, STI);
}

const TargetRegisterClass *
MaxisSETargetLowering::getRepRegClassFor(MVT VT) const {
  if (VT == MVT::Untyped)
    return Subtarget.hasDSP() ? &Maxis::ACC64DSPRegClass : &Maxis::ACC64RegClass;

  return TargetLowering::getRepRegClassFor(VT);
}

// Enable MSA support for the given integer type and Register class.
void MaxisSETargetLowering::
...
SDValue MaxisSETargetLowering::lowerSELECT(SDValue Op, SelectionDAG &DAG) const {
  if(!Subtarget.hasMaxis32r6())
    return MaxisTargetLowering::LowerOperation(Op, DAG);
  ...
  SDValue Tmp = DAG.getNode(MaxisISD::MTC1_D64, DL, MVT::f64, Op->getOperand(0));
  return DAG.getNode(MaxisISD::FSELECT, DL, ResTy, Tmp, Op->getOperand(1),
                     Op->getOperand(2));
}

bool
MaxisSETargetLowering::allowsMisalignedMemoryAccesses(EVT VT,
                                                     unsigned,
                                                     unsigned,
                                                     bool *Fast) const {
  ...
  if (Subtarget.systemSupportsUnalignedAccess()) {
    // MAXIS32r6/MAXIS64r6 is required to support unaligned access. It's
...
SDValue MaxisSETargetLowering::LowerOperation(SDValue Op,
                                             SelectionDAG &DAG) const {
  switch(Op.getOpcode()) {
  case ISD::LOAD:  return lowerLOAD(Op, DAG);
  case ISD::STORE: return lowerSTORE(Op, DAG);
  case ISD::SMUL_LOHI: return lowerMulDiv(Op, MaxisISD::Mult, true, true, DAG);
  case ISD::UMUL_LOHI: return lowerMulDiv(Op, MaxisISD::Multu, true, true, DAG);
...
// Fold zero extensions into MaxisISD::VEXTRACT_[SZ]EXT_ELT
//
// Performs the following transformations:
// - Changes MaxisISD::VEXTRACT_[SZ]EXT_ELT to zero extension if its
...
static SDValue performANDCombine(SDNode *N, SelectionDAG &DAG,
  ...
  // (and (MaxisVExtract[SZ]Ext $a, $b, $c), imm:$d)
  // where $d + 1 == 2^n and n == 32
  // or    $d + 1 == 2^n and n <= 32 and ZExt
  // -> (MaxisVExtractZExt $a, $b, $c)
  if (Op0Opcode == MaxisISD::VEXTRACT_SEXT_ELT ||
      Op0Opcode == MaxisISD::VEXTRACT_ZEXT_ELT) {
    ...
    if ((Op0Opcode == MaxisISD::VEXTRACT_ZEXT_ELT && Log2 >= ExtendTySize) ||
        Log2 == ExtendTySize) {
      SDValue Ops[] = { Op0->getOperand(0), Op0->getOperand(1), Op0Op2 };
      return DAG.getNode(MaxisISD::VEXTRACT_ZEXT_ELT, SDLoc(Op0),
                         Op0->getVTList(),
...
//
// This function is quite similar to MaxisSEDAGToDAGISel::selectVSplat. The
...
static SDValue performMULCombine(SDNode *N, SelectionDAG &DAG,
                                 ...
                                 const MaxisSETargetLowering *TL) {
...
static SDValue performSHLCombine(SDNode *N, SelectionDAG &DAG,
  ...
  return performDSPShiftCombine(MaxisISD::SHLL_DSP, N, Ty, DAG, Subtarget);
}

// Fold sign-extensions into MaxisISD::VEXTRACT_[SZ]EXT_ELT for MSA and fold
// constant splats into MaxisISD::SHRA_DSP for DSPr2.
//
// Performs the following transformations:
// - Changes MaxisISD::VEXTRACT_[SZ]EXT_ELT to sign extension if its
//   sign/zero-extension is completely overwritten by the new one performed by
...
static SDValue performSRACombine(SDNode *N, SelectionDAG &DAG,
  ...
  if (Subtarget.hasMSA()) {
    ...
    // (sra (shl (MaxisVExtract[SZ]Ext $a, $b, $c), imm:$d), imm:$d)
    // where $d + sizeof($c) == 32
    // or    $d + sizeof($c) <= 32 and SExt
    // -> (MaxisVExtractSExt $a, $b, $c)
    if (Op0->getOpcode() == ISD::SHL && Op1 == Op0->getOperand(1)) {
      ...
      if (Op0Op0->getOpcode() != MaxisISD::VEXTRACT_SEXT_ELT &&
          Op0Op0->getOpcode() != MaxisISD::VEXTRACT_ZEXT_ELT)
        return SDValue();
      ...
      if (TotalBits == 32 ||
          (Op0Op0->getOpcode() == MaxisISD::VEXTRACT_SEXT_ELT &&
           TotalBits <= 32)) {
        SDValue Ops[] = { Op0Op0->getOperand(0), Op0Op0->getOperand(1),
                          Op0Op0->getOperand(2) };
        return DAG.getNode(MaxisISD::VEXTRACT_SEXT_ELT, SDLoc(Op0Op0),
                           Op0Op0->getVTList(),
  ...
  return performDSPShiftCombine(MaxisISD::SHRA_DSP, N, Ty, DAG, Subtarget);
}
...
static SDValue performSRLCombine(SDNode *N, SelectionDAG &DAG,
  ...
  return performDSPShiftCombine(MaxisISD::SHRL_DSP, N, Ty, DAG, Subtarget);
}
...
static SDValue performSETCCCombine(SDNode *N, SelectionDAG &DAG) {
  ...
  return DAG.getNode(MaxisISD::SETCC_DSP, SDLoc(N), Ty, N->getOperand(0),
...
static SDValue performVSELECTCombine(SDNode *N, SelectionDAG &DAG) {
  ...
  if (Ty.is128BitVector() && Ty.isInteger()) {
    ...
    if (Op1 == Op0Op0 && Op2 == Op0Op1)
      return DAG.getNode(Signed ? MaxisISD::VSMIN : MaxisISD::VUMIN, SDLoc(N),
                         Ty, Op1, Op2);
    else if (Op1 == Op0Op1 && Op2 == Op0Op0)
      return DAG.getNode(Signed ? MaxisISD::VSMAX : MaxisISD::VUMAX, SDLoc(N),
                         Ty, Op1, Op2);
  } else if ((Ty == MVT::v2i16) || (Ty == MVT::v4i8)) {
    SDValue SetCC = N->getOperand(0);

    if (SetCC.getOpcode() != MaxisISD::SETCC_DSP)
      return SDValue();

    return DAG.getNode(MaxisISD::SELECT_CC_DSP, SDLoc(N), Ty,
...
static SDValue performXORCombine(SDNode *N, SelectionDAG &DAG,
  ...
  if (Subtarget.hasMSA() && Ty.is128BitVector() && Ty.isInteger()) {
    ...
    if (NotOp->getOpcode() == ISD::OR)
      return DAG.getNode(MaxisISD::VNOR, SDLoc(N), Ty, NotOp->getOperand(0),
                         NotOp->getOperand(1));
  }
...
SDValue
MaxisSETargetLowering::PerformDAGCombine(SDNode *N, DAGCombinerInfo &DCI) const {
  ...
  return MaxisTargetLowering::PerformDAGCombine(N, DCI);
}

MachineBasicBlock *
MaxisSETargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
                                                  MachineBasicBlock *BB) const {
  switch (MI.getOpcode()) {
  default:
    return MaxisTargetLowering::EmitInstrWithCustomInserter(MI, BB);
  case Maxis::BPOSGE32_PSEUDO:
    return emitBPOSGE32(MI, BB);
  case Maxis::SNZ_B_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Maxis::BNZ_B);
  case Maxis::SNZ_H_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Maxis::BNZ_H);
  case Maxis::SNZ_W_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Maxis::BNZ_W);
  case Maxis::SNZ_D_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Maxis::BNZ_D);
  case Maxis::SNZ_V_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Maxis::BNZ_V);
  case Maxis::SZ_B_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Maxis::BZ_B);
  case Maxis::SZ_H_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Maxis::BZ_H);
  case Maxis::SZ_W_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Maxis::BZ_W);
  case Maxis::SZ_D_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Maxis::BZ_D);
  case Maxis::SZ_V_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Maxis::BZ_V);
  case Maxis::COPY_FW_PSEUDO:
    return emitCOPY_FW(MI, BB);
  case Maxis::COPY_FD_PSEUDO:
    return emitCOPY_FD(MI, BB);
  case Maxis::INSERT_FW_PSEUDO:
    return emitINSERT_FW(MI, BB);
  case Maxis::INSERT_FD_PSEUDO:
    return emitINSERT_FD(MI, BB);
  case Maxis::INSERT_B_VIDX_PSEUDO:
  case Maxis::INSERT_B_VIDX64_PSEUDO:
    return emitINSERT_DF_VIDX(MI, BB, 1, false);
  case Maxis::INSERT_H_VIDX_PSEUDO:
  case Maxis::INSERT_H_VIDX64_PSEUDO:
    return emitINSERT_DF_VIDX(MI, BB, 2, false);
  case Maxis::INSERT_W_VIDX_PSEUDO:
  case Maxis::INSERT_W_VIDX64_PSEUDO:
    return emitINSERT_DF_VIDX(MI, BB, 4, false);
  case Maxis::INSERT_D_VIDX_PSEUDO:
  case Maxis::INSERT_D_VIDX64_PSEUDO:
    return emitINSERT_DF_VIDX(MI, BB, 8, false);
  case Maxis::INSERT_FW_VIDX_PSEUDO:
  case Maxis::INSERT_FW_VIDX64_PSEUDO:
    return emitINSERT_DF_VIDX(MI, BB, 4, true);
  case Maxis::INSERT_FD_VIDX_PSEUDO:
  case Maxis::INSERT_FD_VIDX64_PSEUDO:
    return emitINSERT_DF_VIDX(MI, BB, 8, true);
  case Maxis::FILL_FW_PSEUDO:
    return emitFILL_FW(MI, BB);
  case Maxis::FILL_FD_PSEUDO:
    return emitFILL_FD(MI, BB);
  case Maxis::FEXP2_W_1_PSEUDO:
    return emitFEXP2_W_1(MI, BB);
  case Maxis::FEXP2_D_1_PSEUDO:
    return emitFEXP2_D_1(MI, BB);
  case Maxis::ST_F16:
    return emitST_F16_PSEUDO(MI, BB);
  case Maxis::LD_F16:
    return emitLD_F16_PSEUDO(MI, BB);
  case Maxis::MSA_FP_EXTEND_W_PSEUDO:
    return emitFPEXTEND_PSEUDO(MI, BB, false);
  case Maxis::MSA_FP_ROUND_W_PSEUDO:
    return emitFPROUND_PSEUDO(MI, BB, false);
  case Maxis::MSA_FP_EXTEND_D_PSEUDO:
    return emitFPEXTEND_PSEUDO(MI, BB, true);
  case Maxis::MSA_FP_ROUND_D_PSEUDO:
    return emitFPROUND_PSEUDO(MI, BB, true);
  }
}

bool MaxisSETargetLowering::isEligibleForTailCallOptimization(
    const CCState &CCInfo, unsigned NextStackOffset,
    const MaxisFunctionInfo &FI) const {
  if (!UseMaxisTailCalls)
    return false;
...
SDValue MaxisSETargetLowering::lowerLOAD(SDValue Op, SelectionDAG &DAG) const {
  LoadSDNode &Nd = *cast<LoadSDNode>(Op);

  if (Nd.getMemoryVT() != MVT::f64 || !NoDPLoadStore)
    return MaxisTargetLowering::lowerLOAD(Op, DAG);
  ...
  SDValue BP = DAG.getNode(MaxisISD::BuildPairF64, DL, MVT::f64, Lo, Hi);
  SDValue Ops[2] = {BP, Hi.getValue(1)};
  return DAG.getMergeValues(Ops, DL);
}

SDValue MaxisSETargetLowering::lowerSTORE(SDValue Op, SelectionDAG &DAG) const {
  StoreSDNode &Nd = *cast<StoreSDNode>(Op);

  if (Nd.getMemoryVT() != MVT::f64 || !NoDPLoadStore)
    return MaxisTargetLowering::lowerSTORE(Op, DAG);
...
SDValue MaxisSETargetLowering::lowerMulDiv(SDValue Op, unsigned NewOpc,
                                          bool HasLo, bool HasHi,
                                          SelectionDAG &DAG) const {
  // MAXIS32r6/MAXIS64r6 removed accumulator based multiplies.
  assert(!Subtarget.hasMaxis32r6());
  ...
  if (HasLo)
    Lo = DAG.getNode(MaxisISD::MFLO, DL, Ty, Mult);
  if (HasHi)
    Hi = DAG.getNode(MaxisISD::MFHI, DL, Ty, Mult);
...
static SDValue initAccumulator(SDValue In, const SDLoc &DL, SelectionDAG &DAG) {
  ...
  return DAG.getNode(MaxisISD::MTLOHI, DL, MVT::Untyped, InLo, InHi);
}

static SDValue extractLOHI(SDValue Op, const SDLoc &DL, SelectionDAG &DAG) {
  SDValue Lo = DAG.getNode(MaxisISD::MFLO, DL, MVT::i32, Op);
  SDValue Hi = DAG.getNode(MaxisISD::MFHI, DL, MVT::i32, Op);
  return DAG.getNode(ISD::BUILD_PAIR, DL, MVT::i64, Lo, Hi);
}

// This function expands maxis intrinsic nodes which have 64-bit input operands
// or output values.
//
...
SDValue MaxisSETargetLowering::lowerINTRINSIC_WO_CHAIN(SDValue Op,
                                                      SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  default:
    return SDValue();
  case Intrinsic::maxis_shilo:
    return lowerDSPIntr(Op, DAG, MaxisISD::SHILO);
  case Intrinsic::maxis_dpau_h_qbl:
    return lowerDSPIntr(Op, DAG, MaxisISD::DPAU_H_QBL);
  case Intrinsic::maxis_dpau_h_qbr:
    return lowerDSPIntr(Op, DAG, MaxisISD::DPAU_H_QBR);
  case Intrinsic::maxis_dpsu_h_qbl:
    return lowerDSPIntr(Op, DAG, MaxisISD::DPSU_H_QBL);
  case Intrinsic::maxis_dpsu_h_qbr:
    return lowerDSPIntr(Op, DAG, MaxisISD::DPSU_H_QBR);
  case Intrinsic::maxis_dpa_w_ph:
    return lowerDSPIntr(Op, DAG, MaxisISD::DPA_W_PH);
  case Intrinsic::maxis_dps_w_ph:
    return lowerDSPIntr(Op, DAG, MaxisISD::DPS_W_PH);
  case Intrinsic::maxis_dpax_w_ph:
    return lowerDSPIntr(Op, DAG, MaxisISD::DPAX_W_PH);
  case Intrinsic::maxis_dpsx_w_ph:
    return lowerDSPIntr(Op, DAG, MaxisISD::DPSX_W_PH);
  case Intrinsic::maxis_mulsa_w_ph:
    return lowerDSPIntr(Op, DAG, MaxisISD::MULSA_W_PH);
  case Intrinsic::maxis_mult:
    return lowerDSPIntr(Op, DAG, MaxisISD::Mult);
  case Intrinsic::maxis_multu:
    return lowerDSPIntr(Op, DAG, MaxisISD::Multu);
  case Intrinsic::maxis_madd:
    return lowerDSPIntr(Op, DAG, MaxisISD::MAdd);
  case Intrinsic::maxis_maddu:
    return lowerDSPIntr(Op, DAG, MaxisISD::MAddu);
  case Intrinsic::maxis_msub:
    return lowerDSPIntr(Op, DAG, MaxisISD::MSub);
  case Intrinsic::maxis_msubu:
    return lowerDSPIntr(Op, DAG, MaxisISD::MSubu);
  case Intrinsic::maxis_addv_b:
  case Intrinsic::maxis_addv_h:
  case Intrinsic::maxis_addv_w:
  case Intrinsic::maxis_addv_d:
    return DAG.getNode(ISD::ADD, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::maxis_addvi_b:
  case Intrinsic::maxis_addvi_h:
  case Intrinsic::maxis_addvi_w:
  case Intrinsic::maxis_addvi_d:
    return DAG.getNode(ISD::ADD, DL, Op->getValueType(0), Op->getOperand(1),
                       lowerMSASplatImm(Op, 2, DAG));
  case Intrinsic::maxis_and_v:
    return DAG.getNode(ISD::AND, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::maxis_andi_b:
    return DAG.getNode(ISD::AND, DL, Op->getValueType(0), Op->getOperand(1),
                       lowerMSASplatImm(Op, 2, DAG));
  case Intrinsic::maxis_bclr_b:
  case Intrinsic::maxis_bclr_h:
  case Intrinsic::maxis_bclr_w:
  case Intrinsic::maxis_bclr_d:
    return lowerMSABitClear(Op, DAG);
  case Intrinsic::maxis_bclri_b:
  case Intrinsic::maxis_bclri_h:
  case Intrinsic::maxis_bclri_w:
  case Intrinsic::maxis_bclri_d:
    return lowerMSABitClearImm(Op, DAG);
  case Intrinsic::maxis_binsli_b:
  case Intrinsic::maxis_binsli_h:
  case Intrinsic::maxis_binsli_w:
  case Intrinsic::maxis_binsli_d: {
  ...
  case Intrinsic::maxis_binsri_b:
  case Intrinsic::maxis_binsri_h:
  case Intrinsic::maxis_binsri_w:
  case Intrinsic::maxis_binsri_d: {
  ...
  case Intrinsic::maxis_bmnz_v:
    return DAG.getNode(ISD::VSELECT, DL, Op->getValueType(0), Op->getOperand(3),
                       Op->getOperand(2), Op->getOperand(1));
  case Intrinsic::maxis_bmnzi_b:
    return DAG.getNode(ISD::VSELECT, DL, Op->getValueType(0),
                       lowerMSASplatImm(Op, 3, DAG), Op->getOperand(2),
                       Op->getOperand(1));
  case Intrinsic::maxis_bmz_v:
    return DAG.getNode(ISD::VSELECT, DL, Op->getValueType(0), Op->getOperand(3),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::maxis_bmzi_b:
    return DAG.getNode(ISD::VSELECT, DL, Op->getValueType(0),
                       lowerMSASplatImm(Op, 3, DAG), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::maxis_bneg_b:
  case Intrinsic::maxis_bneg_h:
  case Intrinsic::maxis_bneg_w:
  case Intrinsic::maxis_bneg_d: {
    EVT VecTy = Op->getValueType(0);
  ...
  case Intrinsic::maxis_bnegi_b:
  case Intrinsic::maxis_bnegi_h:
  case Intrinsic::maxis_bnegi_w:
  case Intrinsic::maxis_bnegi_d:
    return lowerMSABinaryBitImmIntr(Op, DAG, ISD::XOR, Op->getOperand(2),
                                    !Subtarget.isLittle());
  case Intrinsic::maxis_bnz_b:
  case Intrinsic::maxis_bnz_h:
  case Intrinsic::maxis_bnz_w:
  case Intrinsic::maxis_bnz_d:
    return DAG.getNode(MaxisISD::VALL_NONZERO, DL, Op->getValueType(0),
                       Op->getOperand(1));
  case Intrinsic::maxis_bnz_v:
    return DAG.getNode(MaxisISD::VANY_NONZERO, DL, Op->getValueType(0),
                       Op->getOperand(1));
  case Intrinsic::maxis_bsel_v:
    // bsel_v(Mask, IfClear, IfSet) -> (vselect Mask, IfSet, IfClear)
    return DAG.getNode(ISD::VSELECT, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(3),
                       Op->getOperand(2));
  case Intrinsic::maxis_bseli_b:
    // bseli_v(Mask, IfClear, IfSet) -> (vselect Mask, IfSet, IfClear)
    return DAG.getNode(ISD::VSELECT, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 3, DAG),
                       Op->getOperand(2));
  case Intrinsic::maxis_bset_b:
  case Intrinsic::maxis_bset_h:
  case Intrinsic::maxis_bset_w:
  case Intrinsic::maxis_bset_d: {
    EVT VecTy = Op->getValueType(0);
    SDValue One = DAG.getConstant(1, DL, VecTy);
  ...
  case Intrinsic::maxis_bseti_b:
  case Intrinsic::maxis_bseti_h:
  case Intrinsic::maxis_bseti_w:
  case Intrinsic::maxis_bseti_d:
    return lowerMSABinaryBitImmIntr(Op, DAG, ISD::OR, Op->getOperand(2),
                                    !Subtarget.isLittle());
  case Intrinsic::maxis_bz_b:
  case Intrinsic::maxis_bz_h:
  case Intrinsic::maxis_bz_w:
  case Intrinsic::maxis_bz_d:
    return DAG.getNode(MaxisISD::VALL_ZERO, DL, Op->getValueType(0),
                       Op->getOperand(1));
  case Intrinsic::maxis_bz_v:
    return DAG.getNode(MaxisISD::VANY_ZERO, DL, Op->getValueType(0),
                       Op->getOperand(1));
  case Intrinsic::maxis_ceq_b:
  case Intrinsic::maxis_ceq_h:
  case Intrinsic::maxis_ceq_w:
  case Intrinsic::maxis_ceq_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETEQ);
  case Intrinsic::maxis_ceqi_b:
  case Intrinsic::maxis_ceqi_h:
  case Intrinsic::maxis_ceqi_w:
  case Intrinsic::maxis_ceqi_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        lowerMSASplatImm(Op, 2, DAG, true), ISD::SETEQ);
  case Intrinsic::maxis_cle_s_b:
  case Intrinsic::maxis_cle_s_h:
  case Intrinsic::maxis_cle_s_w:
  case Intrinsic::maxis_cle_s_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETLE);
  case Intrinsic::maxis_clei_s_b:
  case Intrinsic::maxis_clei_s_h:
  case Intrinsic::maxis_clei_s_w:
  case Intrinsic::maxis_clei_s_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        lowerMSASplatImm(Op, 2, DAG, true), ISD::SETLE);
  case Intrinsic::maxis_cle_u_b:
  case Intrinsic::maxis_cle_u_h:
  case Intrinsic::maxis_cle_u_w:
  case Intrinsic::maxis_cle_u_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETULE);
  case Intrinsic::maxis_clei_u_b:
  case Intrinsic::maxis_clei_u_h:
  case Intrinsic::maxis_clei_u_w:
  case Intrinsic::maxis_clei_u_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        lowerMSASplatImm(Op, 2, DAG), ISD::SETULE);
  case Intrinsic::maxis_clt_s_b:
  case Intrinsic::maxis_clt_s_h:
  case Intrinsic::maxis_clt_s_w:
  case Intrinsic::maxis_clt_s_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETLT);
  case Intrinsic::maxis_clti_s_b:
  case Intrinsic::maxis_clti_s_h:
  case Intrinsic::maxis_clti_s_w:
  case Intrinsic::maxis_clti_s_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        lowerMSASplatImm(Op, 2, DAG, true), ISD::SETLT);
  case Intrinsic::maxis_clt_u_b:
  case Intrinsic::maxis_clt_u_h:
  case Intrinsic::maxis_clt_u_w:
  case Intrinsic::maxis_clt_u_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETULT);
  case Intrinsic::maxis_clti_u_b:
  case Intrinsic::maxis_clti_u_h:
  case Intrinsic::maxis_clti_u_w:
  case Intrinsic::maxis_clti_u_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        lowerMSASplatImm(Op, 2, DAG), ISD::SETULT);
  case Intrinsic::maxis_copy_s_b:
  case Intrinsic::maxis_copy_s_h:
  case Intrinsic::maxis_copy_s_w:
    return lowerMSACopyIntr(Op, DAG, MaxisISD::VEXTRACT_SEXT_ELT);
  case Intrinsic::maxis_copy_s_d:
    if (Subtarget.hasMaxis64())
      // Lower directly into VEXTRACT_SEXT_ELT since i64 is legal on Maxis64.
      return lowerMSACopyIntr(Op, DAG, MaxisISD::VEXTRACT_SEXT_ELT);
    else {
  ...
  case Intrinsic::maxis_copy_u_b:
  case Intrinsic::maxis_copy_u_h:
  case Intrinsic::maxis_copy_u_w:
    return lowerMSACopyIntr(Op, DAG, MaxisISD::VEXTRACT_ZEXT_ELT);
  case Intrinsic::maxis_copy_u_d:
    if (Subtarget.hasMaxis64())
      // Lower directly into VEXTRACT_ZEXT_ELT since i64 is legal on Maxis64.
      return lowerMSACopyIntr(Op, DAG, MaxisISD::VEXTRACT_ZEXT_ELT);
    else {
      // Lower into the generic EXTRACT_VECTOR_ELT node and let the type
      // legalizer and EXTRACT_VECTOR_ELT lowering sort it out.
  ...
  case Intrinsic::maxis_div_s_b:
  case Intrinsic::maxis_div_s_h:
  case Intrinsic::maxis_div_s_w:
  case Intrinsic::maxis_div_s_d:
    return DAG.getNode(ISD::SDIV, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::maxis_div_u_b:
  case Intrinsic::maxis_div_u_h:
  case Intrinsic::maxis_div_u_w:
  case Intrinsic::maxis_div_u_d:
    return DAG.getNode(ISD::UDIV, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::maxis_fadd_w:
  case Intrinsic::maxis_fadd_d:
    // TODO: If intrinsics have fast-math-flags, propagate them.
    return DAG.getNode(ISD::FADD, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  // Don't lower maxis_fcaf_[wd] since LLVM folds SETFALSE condcodes away
  case Intrinsic::maxis_fceq_w:
  case Intrinsic::maxis_fceq_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETOEQ);
  case Intrinsic::maxis_fcle_w:
  case Intrinsic::maxis_fcle_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETOLE);
  case Intrinsic::maxis_fclt_w:
  case Intrinsic::maxis_fclt_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETOLT);
  case Intrinsic::maxis_fcne_w:
  case Intrinsic::maxis_fcne_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETONE);
  case Intrinsic::maxis_fcor_w:
  case Intrinsic::maxis_fcor_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETO);
  case Intrinsic::maxis_fcueq_w:
  case Intrinsic::maxis_fcueq_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETUEQ);
  case Intrinsic::maxis_fcule_w:
  case Intrinsic::maxis_fcule_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETULE);
  case Intrinsic::maxis_fcult_w:
  case Intrinsic::maxis_fcult_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETULT);
  case Intrinsic::maxis_fcun_w:
  case Intrinsic::maxis_fcun_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETUO);
  case Intrinsic::maxis_fcune_w:
  case Intrinsic::maxis_fcune_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETUNE);
  case Intrinsic::maxis_fdiv_w:
  case Intrinsic::maxis_fdiv_d:
    // TODO: If intrinsics have fast-math-flags, propagate them.
    return DAG.getNode(ISD::FDIV, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::maxis_ffint_u_w:
  case Intrinsic::maxis_ffint_u_d:
    return DAG.getNode(ISD::UINT_TO_FP, DL, Op->getValueType(0),
                       Op->getOperand(1));
  case Intrinsic::maxis_ffint_s_w:
  case Intrinsic::maxis_ffint_s_d:
    return DAG.getNode(ISD::SINT_TO_FP, DL, Op->getValueType(0),
                       Op->getOperand(1));
  case Intrinsic::maxis_fill_b:
  case Intrinsic::maxis_fill_h:
  case Intrinsic::maxis_fill_w:
  case Intrinsic::maxis_fill_d: {
    EVT ResTy = Op->getValueType(0);
  ...
  case Intrinsic::maxis_flog2_w:
  case Intrinsic::maxis_flog2_d:
    return DAG.getNode(ISD::FLOG2, DL, Op->getValueType(0), Op->getOperand(1));
  case Intrinsic::maxis_fmadd_w:
  case Intrinsic::maxis_fmadd_d:
    return DAG.getNode(ISD::FMA, SDLoc(Op), Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2), Op->getOperand(3));
  case Intrinsic::maxis_fmul_w:
  case Intrinsic::maxis_fmul_d:
    // TODO: If intrinsics have fast-math-flags, propagate them.
    return DAG.getNode(ISD::FMUL, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::maxis_fmsub_w:
  case Intrinsic::maxis_fmsub_d: {
    // TODO: If intrinsics have fast-math-flags, propagate them.
    EVT ResTy = Op->getValueType(0);
  ...
  case Intrinsic::maxis_frint_w:
  case Intrinsic::maxis_frint_d:
    return DAG.getNode(ISD::FRINT, DL, Op->getValueType(0), Op->getOperand(1));
  case Intrinsic::maxis_fsqrt_w:
  case Intrinsic::maxis_fsqrt_d:
    return DAG.getNode(ISD::FSQRT, DL, Op->getValueType(0), Op->getOperand(1));
  case Intrinsic::maxis_fsub_w:
  case Intrinsic::maxis_fsub_d:
    // TODO: If intrinsics have fast-math-flags, propagate them.
    return DAG.getNode(ISD::FSUB, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::maxis_ftrunc_u_w:
  case Intrinsic::maxis_ftrunc_u_d:
    return DAG.getNode(ISD::FP_TO_UINT, DL, Op->getValueType(0),
                       Op->getOperand(1));
  case Intrinsic::maxis_ftrunc_s_w:
  case Intrinsic::maxis_ftrunc_s_d:
    return DAG.getNode(ISD::FP_TO_SINT, DL, Op->getValueType(0),
                       Op->getOperand(1));
  case Intrinsic::maxis_ilvev_b:
  case Intrinsic::maxis_ilvev_h:
  case Intrinsic::maxis_ilvev_w:
  case Intrinsic::maxis_ilvev_d:
    return DAG.getNode(MaxisISD::ILVEV, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::maxis_ilvl_b:
  case Intrinsic::maxis_ilvl_h:
  case Intrinsic::maxis_ilvl_w:
  case Intrinsic::maxis_ilvl_d:
    return DAG.getNode(MaxisISD::ILVL, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::maxis_ilvod_b:
  case Intrinsic::maxis_ilvod_h:
  case Intrinsic::maxis_ilvod_w:
  case Intrinsic::maxis_ilvod_d:
    return DAG.getNode(MaxisISD::ILVOD, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::maxis_ilvr_b:
  case Intrinsic::maxis_ilvr_h:
  case Intrinsic::maxis_ilvr_w:
  case Intrinsic::maxis_ilvr_d:
    return DAG.getNode(MaxisISD::ILVR, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::maxis_insert_b:
  case Intrinsic::maxis_insert_h:
  case Intrinsic::maxis_insert_w:
  case Intrinsic::maxis_insert_d:
    return DAG.getNode(ISD::INSERT_VECTOR_ELT, SDLoc(Op), Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(3), Op->getOperand(2));
  case Intrinsic::maxis_insve_b:
  case Intrinsic::maxis_insve_h:
  case Intrinsic::maxis_insve_w:
  case Intrinsic::maxis_insve_d: {
    // Report an error for out of range values.
    int64_t Max;
    switch (Intrinsic) {
    case Intrinsic::maxis_insve_b: Max = 15; break;
    case Intrinsic::maxis_insve_h: Max = 7; break;
    case Intrinsic::maxis_insve_w: Max = 3; break;
    case Intrinsic::maxis_insve_d: Max = 1; break;
    default: llvm_unreachable("Unmatched intrinsic");
    }
    int64_t Value = cast<ConstantSDNode>(Op->getOperand(2))->getSExtValue();
    if (Value < 0 || Value > Max)
      report_fatal_error("Immediate out of range");
    return DAG.getNode(MaxisISD::INSVE, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2), Op->getOperand(3),
                       DAG.getConstant(0, DL, MVT::i32));
    }
  case Intrinsic::maxis_ldi_b:
  case Intrinsic::maxis_ldi_h:
  case Intrinsic::maxis_ldi_w:
  case Intrinsic::maxis_ldi_d:
    return lowerMSASplatImm(Op, 1, DAG, true);
  case Intrinsic::maxis_lsa:
  case Intrinsic::maxis_dlsa: {
    EVT ResTy = Op->getValueType(0);
    return DAG.getNode(ISD::ADD, SDLoc(Op), ResTy, Op->getOperand(1),
                       DAG.getNode(ISD::SHL, SDLoc(Op), ResTy,
                                   Op->getOperand(2), Op->getOperand(3)));
  }
  case Intrinsic::maxis_maddv_b:
  case Intrinsic::maxis_maddv_h:
  case Intrinsic::maxis_maddv_w:
  case Intrinsic::maxis_maddv_d: {
    EVT ResTy = Op->getValueType(0);
    return DAG.getNode(ISD::ADD, SDLoc(Op), ResTy, Op->getOperand(1),
                       DAG.getNode(ISD::MUL, SDLoc(Op), ResTy,
                                   Op->getOperand(2), Op->getOperand(3)));
  }
  case Intrinsic::maxis_max_s_b:
  case Intrinsic::maxis_max_s_h:
  case Intrinsic::maxis_max_s_w:
  case Intrinsic::maxis_max_s_d:
    return DAG.getNode(MaxisISD::VSMAX, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::maxis_max_u_b:
  case Intrinsic::maxis_max_u_h:
  case Intrinsic::maxis_max_u_w:
  case Intrinsic::maxis_max_u_d:
    return DAG.getNode(MaxisISD::VUMAX, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::maxis_maxi_s_b:
  case Intrinsic::maxis_maxi_s_h:
  case Intrinsic::maxis_maxi_s_w:
  case Intrinsic::maxis_maxi_s_d:
    return DAG.getNode(MaxisISD::VSMAX, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG, true));
  case Intrinsic::maxis_maxi_u_b:
  case Intrinsic::maxis_maxi_u_h:
  case Intrinsic::maxis_maxi_u_w:
  case Intrinsic::maxis_maxi_u_d:
    return DAG.getNode(MaxisISD::VUMAX, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG));
  case Intrinsic::maxis_min_s_b:
  case Intrinsic::maxis_min_s_h:
  case Intrinsic::maxis_min_s_w:
  case Intrinsic::maxis_min_s_d:
    return DAG.getNode(MaxisISD::VSMIN, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::maxis_min_u_b:
  case Intrinsic::maxis_min_u_h:
  case Intrinsic::maxis_min_u_w:
  case Intrinsic::maxis_min_u_d:
    return DAG.getNode(MaxisISD::VUMIN, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::maxis_mini_s_b:
  case Intrinsic::maxis_mini_s_h:
  case Intrinsic::maxis_mini_s_w:
  case Intrinsic::maxis_mini_s_d:
    return DAG.getNode(MaxisISD::VSMIN, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG, true));
  case Intrinsic::maxis_mini_u_b:
  case Intrinsic::maxis_mini_u_h:
  case Intrinsic::maxis_mini_u_w:
  case Intrinsic::maxis_mini_u_d:
    return DAG.getNode(MaxisISD::VUMIN, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG));
  case Intrinsic::maxis_mod_s_b:
  case Intrinsic::maxis_mod_s_h:
  case Intrinsic::maxis_mod_s_w:
  case Intrinsic::maxis_mod_s_d:
    return DAG.getNode(ISD::SREM, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::maxis_mod_u_b:
  case Intrinsic::maxis_mod_u_h:
  case Intrinsic::maxis_mod_u_w:
  case Intrinsic::maxis_mod_u_d:
    return DAG.getNode(ISD::UREM, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::maxis_mulv_b:
  case Intrinsic::maxis_mulv_h:
  case Intrinsic::maxis_mulv_w:
  case Intrinsic::maxis_mulv_d:
    return DAG.getNode(ISD::MUL, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::maxis_msubv_b:
  case Intrinsic::maxis_msubv_h:
  case Intrinsic::maxis_msubv_w:
  case Intrinsic::maxis_msubv_d: {
    EVT ResTy = Op->getValueType(0);
    return DAG.getNode(ISD::SUB, SDLoc(Op), ResTy, Op->getOperand(1),
                       DAG.getNode(ISD::MUL, SDLoc(Op), ResTy,
                                   Op->getOperand(2), Op->getOperand(3)));
  }
  case Intrinsic::maxis_nlzc_b:
  case Intrinsic::maxis_nlzc_h:
  case Intrinsic::maxis_nlzc_w:
  case Intrinsic::maxis_nlzc_d:
    return DAG.getNode(ISD::CTLZ, DL, Op->getValueType(0), Op->getOperand(1));
  case Intrinsic::maxis_nor_v: {
    SDValue Res = DAG.getNode(ISD::OR, DL, Op->getValueType(0),
                              Op->getOperand(1), Op->getOperand(2));
    return DAG.getNOT(DL, Res, Res->getValueType(0));
  }
  case Intrinsic::maxis_nori_b: {
    SDValue Res =  DAG.getNode(ISD::OR, DL, Op->getValueType(0),
                               Op->getOperand(1),
                               lowerMSASplatImm(Op, 2, DAG));
    return DAG.getNOT(DL, Res, Res->getValueType(0));
  }
  case Intrinsic::maxis_or_v:
    return DAG.getNode(ISD::OR, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::maxis_ori_b:
    return DAG.getNode(ISD::OR, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG));
  case Intrinsic::maxis_pckev_b:
  case Intrinsic::maxis_pckev_h:
  case Intrinsic::maxis_pckev_w:
  case Intrinsic::maxis_pckev_d:
    return DAG.getNode(MaxisISD::PCKEV, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::maxis_pckod_b:
  case Intrinsic::maxis_pckod_h:
  case Intrinsic::maxis_pckod_w:
  case Intrinsic::maxis_pckod_d:
    return DAG.getNode(MaxisISD::PCKOD, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::maxis_pcnt_b:
  case Intrinsic::maxis_pcnt_h:
  case Intrinsic::maxis_pcnt_w:
  case Intrinsic::maxis_pcnt_d:
    return DAG.getNode(ISD::CTPOP, DL, Op->getValueType(0), Op->getOperand(1));
  case Intrinsic::maxis_sat_s_b:
  case Intrinsic::maxis_sat_s_h:
  case Intrinsic::maxis_sat_s_w:
  case Intrinsic::maxis_sat_s_d:
  case Intrinsic::maxis_sat_u_b:
  case Intrinsic::maxis_sat_u_h:
  case Intrinsic::maxis_sat_u_w:
  case Intrinsic::maxis_sat_u_d: {
    // Report an error for out of range values.
    int64_t Max;
    switch (Intrinsic) {
    case Intrinsic::maxis_sat_s_b:
    case Intrinsic::maxis_sat_u_b: Max = 7;  break;
    case Intrinsic::maxis_sat_s_h:
    case Intrinsic::maxis_sat_u_h: Max = 15; break;
    case Intrinsic::maxis_sat_s_w:
    case Intrinsic::maxis_sat_u_w: Max = 31; break;
    case Intrinsic::maxis_sat_s_d:
    case Intrinsic::maxis_sat_u_d: Max = 63; break;
    default: llvm_unreachable("Unmatched intrinsic");
    }
    int64_t Value = cast<ConstantSDNode>(Op->getOperand(2))->getSExtValue();
    if (Value < 0 || Value > Max)
  ...
  case Intrinsic::maxis_shf_b:
  case Intrinsic::maxis_shf_h:
  case Intrinsic::maxis_shf_w: {
    int64_t Value = cast<ConstantSDNode>(Op->getOperand(2))->getSExtValue();
    if (Value < 0 || Value > 255)
      report_fatal_error("Immediate out of range");
    return DAG.getNode(MaxisISD::SHF, DL, Op->getValueType(0),
                       Op->getOperand(2), Op->getOperand(1));
  }
  case Intrinsic::maxis_sldi_b:
  case Intrinsic::maxis_sldi_h:
  case Intrinsic::maxis_sldi_w:
  case Intrinsic::maxis_sldi_d: {
    // Report an error for out of range values.
    int64_t Max;
    switch (Intrinsic) {
    case Intrinsic::maxis_sldi_b: Max = 15; break;
    case Intrinsic::maxis_sldi_h: Max = 7; break;
    case Intrinsic::maxis_sldi_w: Max = 3; break;
    case Intrinsic::maxis_sldi_d: Max = 1; break;
    default: llvm_unreachable("Unmatched intrinsic");
    }
  ...
  case Intrinsic::maxis_sll_b:
  case Intrinsic::maxis_sll_h:
  case Intrinsic::maxis_sll_w:
  case Intrinsic::maxis_sll_d:
    return DAG.getNode(ISD::SHL, DL, Op->getValueType(0), Op->getOperand(1),
                       truncateVecElts(Op, DAG));
  case Intrinsic::maxis_slli_b:
  case Intrinsic::maxis_slli_h:
  case Intrinsic::maxis_slli_w:
  case Intrinsic::maxis_slli_d:
    return DAG.getNode(ISD::SHL, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG));
  case Intrinsic::maxis_splat_b:
  case Intrinsic::maxis_splat_h:
  case Intrinsic::maxis_splat_w:
  case Intrinsic::maxis_splat_d:
    ...
    // EXTRACT_VECTOR_ELT can't extract i64's on MAXIS32.
    // Instead we lower to MaxisISD::VSHF and match from there.
    return DAG.getNode(MaxisISD::VSHF, DL, Op->getValueType(0),
                       lowerMSASplatZExt(Op, 2, DAG), Op->getOperand(1),
                       Op->getOperand(1));
  case Intrinsic::maxis_splati_b:
  case Intrinsic::maxis_splati_h:
  case Intrinsic::maxis_splati_w:
  case Intrinsic::maxis_splati_d:
    return DAG.getNode(MaxisISD::VSHF, DL, Op->getValueType(0),
                       lowerMSASplatImm(Op, 2, DAG), Op->getOperand(1),
                       Op->getOperand(1));
  case Intrinsic::maxis_sra_b:
  case Intrinsic::maxis_sra_h:
  case Intrinsic::maxis_sra_w:
  case Intrinsic::maxis_sra_d:
    return DAG.getNode(ISD::SRA, DL, Op->getValueType(0), Op->getOperand(1),
                       truncateVecElts(Op, DAG));
  case Intrinsic::maxis_srai_b:
  case Intrinsic::maxis_srai_h:
  case Intrinsic::maxis_srai_w:
  case Intrinsic::maxis_srai_d:
    return DAG.getNode(ISD::SRA, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG));
  case Intrinsic::maxis_srari_b:
  case Intrinsic::maxis_srari_h:
  case Intrinsic::maxis_srari_w:
  case Intrinsic::maxis_srari_d: {
    // Report an error for out of range values.
    int64_t Max;
    switch (Intrinsic) {
    case Intrinsic::maxis_srari_b: Max = 7; break;
    case Intrinsic::maxis_srari_h: Max = 15; break;
    case Intrinsic::maxis_srari_w: Max = 31; break;
    case Intrinsic::maxis_srari_d: Max = 63; break;
    default: llvm_unreachable("Unmatched intrinsic");
    }
    int64_t Value = cast<ConstantSDNode>(Op->getOperand(2))->getSExtValue();
    if (Value < 0 || Value > Max)
  ...
  case Intrinsic::maxis_srl_b:
  case Intrinsic::maxis_srl_h:
  case Intrinsic::maxis_srl_w:
  case Intrinsic::maxis_srl_d:
    return DAG.getNode(ISD::SRL, DL, Op->getValueType(0), Op->getOperand(1),
                       truncateVecElts(Op, DAG));
  case Intrinsic::maxis_srli_b:
  case Intrinsic::maxis_srli_h:
  case Intrinsic::maxis_srli_w:
  case Intrinsic::maxis_srli_d:
    return DAG.getNode(ISD::SRL, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG));
  case Intrinsic::maxis_srlri_b:
  case Intrinsic::maxis_srlri_h:
  case Intrinsic::maxis_srlri_w:
  case Intrinsic::maxis_srlri_d: {
    // Report an error for out of range values.
    int64_t Max;
    switch (Intrinsic) {
    case Intrinsic::maxis_srlri_b: Max = 7; break;
    case Intrinsic::maxis_srlri_h: Max = 15; break;
    case Intrinsic::maxis_srlri_w: Max = 31; break;
    case Intrinsic::maxis_srlri_d: Max = 63; break;
    default: llvm_unreachable("Unmatched intrinsic");
    }
    int64_t Value = cast<ConstantSDNode>(Op->getOperand(2))->getSExtValue();
    if (Value < 0 || Value > Max)
  ...
  case Intrinsic::maxis_subv_b:
  case Intrinsic::maxis_subv_h:
  case Intrinsic::maxis_subv_w:
  case Intrinsic::maxis_subv_d:
    return DAG.getNode(ISD::SUB, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::maxis_subvi_b:
  case Intrinsic::maxis_subvi_h:
  case Intrinsic::maxis_subvi_w:
  case Intrinsic::maxis_subvi_d:
    return DAG.getNode(ISD::SUB, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG));
  case Intrinsic::maxis_vshf_b:
  case Intrinsic::maxis_vshf_h:
  case Intrinsic::maxis_vshf_w:
  case Intrinsic::maxis_vshf_d:
    return DAG.getNode(MaxisISD::VSHF, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2), Op->getOperand(3));
  case Intrinsic::maxis_xor_v:
    return DAG.getNode(ISD::XOR, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::maxis_xori_b:
    return DAG.getNode(ISD::XOR, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG));
  case Intrinsic::thread_pointer: {
    EVT PtrVT = getPointerTy(DAG.getDataLayout());
    return DAG.getNode(MaxisISD::ThreadPointer, DL, PtrVT);
  }
  }
}
...
SDValue MaxisSETargetLowering::lowerINTRINSIC_W_CHAIN(SDValue Op,
                                                     SelectionDAG &DAG) const {
  unsigned Intr = cast<ConstantSDNode>(Op->getOperand(1))->getZExtValue();
  switch (Intr) {
  default:
    return SDValue();
  case Intrinsic::maxis_extp:
    return lowerDSPIntr(Op, DAG, MaxisISD::EXTP);
  case Intrinsic::maxis_extpdp:
    return lowerDSPIntr(Op, DAG, MaxisISD::EXTPDP);
  case Intrinsic::maxis_extr_w:
    return lowerDSPIntr(Op, DAG, MaxisISD::EXTR_W);
  case Intrinsic::maxis_extr_r_w:
    return lowerDSPIntr(Op, DAG, MaxisISD::EXTR_R_W);
  case Intrinsic::maxis_extr_rs_w:
    return lowerDSPIntr(Op, DAG, MaxisISD::EXTR_RS_W);
  case Intrinsic::maxis_extr_s_h:
    return lowerDSPIntr(Op, DAG, MaxisISD::EXTR_S_H);
  case Intrinsic::maxis_mthlip:
    return lowerDSPIntr(Op, DAG, MaxisISD::MTHLIP);
  case Intrinsic::maxis_mulsaq_s_w_ph:
    return lowerDSPIntr(Op, DAG, MaxisISD::MULSAQ_S_W_PH);
  case Intrinsic::maxis_maq_s_w_phl:
    return lowerDSPIntr(Op, DAG, MaxisISD::MAQ_S_W_PHL);
  case Intrinsic::maxis_maq_s_w_phr:
    return lowerDSPIntr(Op, DAG, MaxisISD::MAQ_S_W_PHR);
  case Intrinsic::maxis_maq_sa_w_phl:
    return lowerDSPIntr(Op, DAG, MaxisISD::MAQ_SA_W_PHL);
  case Intrinsic::maxis_maq_sa_w_phr:
    return lowerDSPIntr(Op, DAG, MaxisISD::MAQ_SA_W_PHR);
  case Intrinsic::maxis_dpaq_s_w_ph:
    return lowerDSPIntr(Op, DAG, MaxisISD::DPAQ_S_W_PH);
  case Intrinsic::maxis_dpsq_s_w_ph:
    return lowerDSPIntr(Op, DAG, MaxisISD::DPSQ_S_W_PH);
  case Intrinsic::maxis_dpaq_sa_l_w:
    return lowerDSPIntr(Op, DAG, MaxisISD::DPAQ_SA_L_W);
  case Intrinsic::maxis_dpsq_sa_l_w:
    return lowerDSPIntr(Op, DAG, MaxisISD::DPSQ_SA_L_W);
  case Intrinsic::maxis_dpaqx_s_w_ph:
    return lowerDSPIntr(Op, DAG, MaxisISD::DPAQX_S_W_PH);
  case Intrinsic::maxis_dpaqx_sa_w_ph:
    return lowerDSPIntr(Op, DAG, MaxisISD::DPAQX_SA_W_PH);
  case Intrinsic::maxis_dpsqx_s_w_ph:
    return lowerDSPIntr(Op, DAG, MaxisISD::DPSQX_S_W_PH);
  case Intrinsic::maxis_dpsqx_sa_w_ph:
    return lowerDSPIntr(Op, DAG, MaxisISD::DPSQX_SA_W_PH);
  case Intrinsic::maxis_ld_b:
  case Intrinsic::maxis_ld_h:
  case Intrinsic::maxis_ld_w:
  case Intrinsic::maxis_ld_d:
...
SDValue MaxisSETargetLowering::lowerINTRINSIC_VOID(SDValue Op,
                                                  SelectionDAG &DAG) const {
  unsigned Intr = cast<ConstantSDNode>(Op->getOperand(1))->getZExtValue();
  switch (Intr) {
  default:
    return SDValue();
  case Intrinsic::maxis_st_b:
  case Intrinsic::maxis_st_h:
  case Intrinsic::maxis_st_w:
  case Intrinsic::maxis_st_d:
...
// Lower ISD::EXTRACT_VECTOR_ELT into MaxisISD::VEXTRACT_SEXT_ELT.
//
...
lowerEXTRACT_VECTOR_ELT(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (ResTy.isInteger()) {
    ...
    EVT EltTy = VecTy.getVectorElementType();
    return DAG.getNode(MaxisISD::VEXTRACT_SEXT_ELT, DL, ResTy, Op0, Op1,
                       DAG.getValueType(EltTy));
  }
...
SDValue MaxisSETargetLowering::lowerBUILD_VECTOR(SDValue Op,
                                                SelectionDAG &DAG) const {
...
static SDValue lowerVECTOR_SHUFFLE_SHF(SDValue Op, EVT ResTy,
  ...
  SDLoc DL(Op);
  return DAG.getNode(MaxisISD::SHF, DL, ResTy,
...
static SDValue lowerVECTOR_SHUFFLE_ILVEV(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(MaxisISD::ILVEV, SDLoc(Op), ResTy, Ws, Wt);
}
...
static SDValue lowerVECTOR_SHUFFLE_ILVOD(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(MaxisISD::ILVOD, SDLoc(Op), ResTy, Wt, Ws);
}
...
static SDValue lowerVECTOR_SHUFFLE_ILVR(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(MaxisISD::ILVR, SDLoc(Op), ResTy, Ws, Wt);
}
...
static SDValue lowerVECTOR_SHUFFLE_ILVL(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(MaxisISD::ILVL, SDLoc(Op), ResTy, Ws, Wt);
}
...
static SDValue lowerVECTOR_SHUFFLE_PCKEV(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(MaxisISD::PCKEV, SDLoc(Op), ResTy, Ws, Wt);
}
...
static SDValue lowerVECTOR_SHUFFLE_PCKOD(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(MaxisISD::PCKOD, SDLoc(Op), ResTy, Ws, Wt);
}
...
static SDValue lowerVECTOR_SHUFFLE_VSHF(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(MaxisISD::VSHF, DL, ResTy, MaskVec, Op1, Op0);
}

// Lower VECTOR_SHUFFLE into one of a number of instructions depending on the
// indices in the shuffle.
SDValue MaxisSETargetLowering::lowerVECTOR_SHUFFLE(SDValue Op,
                                                  SelectionDAG &DAG) const {
...
MachineBasicBlock *
MaxisSETargetLowering::emitBPOSGE32(MachineInstr &MI,
                                   MachineBasicBlock *BB) const {
  // $bb:
  //  bposge32_pseudo $vr0
  //  =>
  ...
  BuildMI(BB, DL, TII->get(Maxis::BPOSGE32)).addMBB(TBB);
  // Insert the real bposge32c instruction to $BB.
  BuildMI(BB, DL, TII->get(Maxis::BPOSGE32C_MMR3)).addMBB(TBB);
  ...
  BuildMI(*Sink, Sink->begin(), DL, TII->get(Maxis::PHI),
...
MachineBasicBlock *MaxisSETargetLowering::emitMSACBranchPseudo(
...
MachineBasicBlock *
MaxisSETargetLowering::emitCOPY_FW(MachineInstr &MI,
                                  MachineBasicBlock *BB) const {
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  DebugLoc DL = MI.getDebugLoc();
  ...
  if (Lane == 0) {
    ...
    if (!Subtarget.useOddSPReg()) {
      ...
      Wt = RegInfo.createVirtualRegister(&Maxis::MSA128WEvensRegClass);

      BuildMI(*BB, MI, DL, TII->get(Maxis::COPY), Wt).addReg(Ws);
    }

    BuildMI(*BB, MI, DL, TII->get(Maxis::COPY), Fd).addReg(Wt, 0, Maxis::sub_lo);
  } else {
    unsigned Wt = RegInfo.createVirtualRegister(
        Subtarget.useOddSPReg() ? &Maxis::MSA128WRegClass :
                                  &Maxis::MSA128WEvensRegClass);

    BuildMI(*BB, MI, DL, TII->get(Maxis::SPLATI_W), Wt).addReg(Ws).addImm(Lane);
    BuildMI(*BB, MI, DL, TII->get(Maxis::COPY), Fd).addReg(Wt, 0, Maxis::sub_lo);
  }
...
MachineBasicBlock *
MaxisSETargetLowering::emitCOPY_FD(MachineInstr &MI,
                                  MachineBasicBlock *BB) const {
  ...
  if (Lane == 0)
    BuildMI(*BB, MI, DL, TII->get(Maxis::COPY), Fd).addReg(Ws, 0, Maxis::sub_64);
  else {
    unsigned Wt = RegInfo.createVirtualRegister(&Maxis::MSA128DRegClass);

    BuildMI(*BB, MI, DL, TII->get(Maxis::SPLATI_D), Wt).addReg(Ws).addImm(1);
    BuildMI(*BB, MI, DL, TII->get(Maxis::COPY), Fd).addReg(Wt, 0, Maxis::sub_64);
  }
...
MachineBasicBlock *
MaxisSETargetLowering::emitINSERT_FW(MachineInstr &MI,
                                    MachineBasicBlock *BB) const {
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  DebugLoc DL = MI.getDebugLoc();
  ...
  BuildMI(*BB, MI, DL, TII->get(Maxis::SUBREG_TO_REG), Wt)
      .addImm(0)
      .addReg(Fs)
      .addImm(Maxis::sub_lo);
  BuildMI(*BB, MI, DL, TII->get(Maxis::INSVE_W), Wd)
      .addReg(Wd_in)
      .addImm(Lane)
      .addReg(Wt)
      .addImm(0);
...
MachineBasicBlock *
MaxisSETargetLowering::emitINSERT_FD(MachineInstr &MI,
                                    MachineBasicBlock *BB) const {
  assert(Subtarget.isFP64bit());
  ...
  BuildMI(*BB, MI, DL, TII->get(Maxis::SUBREG_TO_REG), Wt)
      ...
      .addReg(Fs)
      .addImm(Maxis::sub_64);
  BuildMI(*BB, MI, DL, TII->get(Maxis::INSVE_D), Wd)
      .addReg(Wd_in)
      .addImm(Lane)
      .addReg(Wt)
      .addImm(0);
...
// (NEG $lanetmp2, $lanetmp1)
// (SLD_B $wd, $wdtmp2, $wdtmp2,  $lanetmp2)
MachineBasicBlock *MaxisSETargetLowering::emitINSERT_DF_VIDX(
  ...
  case 1:
    EltLog2Size = 0;
    InsertOp = Maxis::INSERT_B;
    InsveOp = Maxis::INSVE_B;
    VecRC = &Maxis::MSA128BRegClass;
    break;
  case 2:
    EltLog2Size = 1;
    InsertOp = Maxis::INSERT_H;
    InsveOp = Maxis::INSVE_H;
    VecRC = &Maxis::MSA128HRegClass;
    break;
  case 4:
    EltLog2Size = 2;
    InsertOp = Maxis::INSERT_W;
    InsveOp = Maxis::INSVE_W;
    VecRC = &Maxis::MSA128WRegClass;
    break;
  case 8:
    EltLog2Size = 3;
    InsertOp = Maxis::INSERT_D;
    InsveOp = Maxis::INSVE_D;
    VecRC = &Maxis::MSA128DRegClass;
    break;
  }
  ...
  if (IsFP) {
    ...
    BuildMI(*BB, MI, DL, TII->get(Maxis::SUBREG_TO_REG), Wt)
        .addImm(0)
        .addReg(SrcValReg)
        .addImm(EltSizeInBytes == 8 ? Maxis::sub_64 : Maxis::sub_lo);
    SrcValReg = Wt;
  }
  ...
  BuildMI(*BB, MI, DL, TII->get(Maxis::SLD_B), WdTmp1)
      .addReg(SrcVecReg)
      .addReg(SrcVecReg)
  ...
  BuildMI(*BB, MI, DL, TII->get(Subtarget.isABI_N64() ? Maxis::DSUB : Maxis::SUB),
          LaneTmp2)
      .addReg(Subtarget.isABI_N64() ? Maxis::ZERO_64 : Maxis::ZERO)
      .addReg(LaneReg);
  BuildMI(*BB, MI, DL, TII->get(Maxis::SLD_B), Wd)
      .addReg(WdTmp2)
      .addReg(WdTmp2)
...
// splati.w $wd, $wt2[0]
MachineBasicBlock *
MaxisSETargetLowering::emitFILL_FW(MachineInstr &MI,
                                  MachineBasicBlock *BB) const {
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  DebugLoc DL = MI.getDebugLoc();
  ...
  BuildMI(*BB, MI, DL, TII->get(Maxis::IMPLICIT_DEF), Wt1);
  BuildMI(*BB, MI, DL, TII->get(Maxis::INSERT_SUBREG), Wt2)
      .addReg(Wt1)
      .addReg(Fs)
      .addImm(Maxis::sub_lo);
  BuildMI(*BB, MI, DL, TII->get(Maxis::SPLATI_W), Wd).addReg(Wt2).addImm(0);
...
// splati.d $wd, $wt2[0]
MachineBasicBlock *
MaxisSETargetLowering::emitFILL_FD(MachineInstr &MI,
                                  MachineBasicBlock *BB) const {
  assert(Subtarget.isFP64bit());
  ...
  unsigned Wd = MI.getOperand(0).getReg();
  unsigned Fs = MI.getOperand(1).getReg();
  unsigned Wt1 = RegInfo.createVirtualRegister(&Maxis::MSA128DRegClass);
  ...
  BuildMI(*BB, MI, DL, TII->get(Maxis::INSERT_SUBREG), Wt2)
      ...
      .addReg(Fs)
      .addImm(Maxis::sub_64);
  BuildMI(*BB, MI, DL, TII->get(Maxis::SPLATI_D), Wd).addReg(Wt2).addImm(0);
...
// space.
MachineBasicBlock *
MaxisSETargetLowering::emitST_F16_PSEUDO(MachineInstr &MI,
                                       MachineBasicBlock *BB) const {
  ...
  const TargetRegisterClass *RC =
      MI.getOperand(1).isReg() ? RegInfo.getRegClass(MI.getOperand(1).getReg())
                               : (Subtarget.isABI_O32() ? &Maxis::GPR32RegClass
                                                        : &Maxis::GPR64RegClass);
  const bool UsingMaxis32 = RC == &Maxis::GPR32RegClass;
  unsigned Rs = RegInfo.createVirtualRegister(&Maxis::GPR32RegClass);

  BuildMI(*BB, MI, DL, TII->get(Maxis::COPY_U_H), Rs).addReg(Ws).addImm(0);
  if(!UsingMaxis32) {
    unsigned Tmp = RegInfo.createVirtualRegister(&Maxis::GPR64RegClass);
    BuildMI(*BB, MI, DL, TII->get(Maxis::SUBREG_TO_REG), Tmp)
        .addImm(0)
        .addReg(Rs)
        .addImm(Maxis::sub_32);
    Rs = Tmp;
  }
  BuildMI(*BB, MI, DL, TII->get(UsingMaxis32 ? Maxis::SH : Maxis::SH64))
      .addReg(Rs)
      .addReg(Rt)
      .addImm(Imm)
...
//     intervention.
MachineBasicBlock *
MaxisSETargetLowering::emitLD_F16_PSEUDO(MachineInstr &MI,
                                       MachineBasicBlock *BB) const {
  ...
  MachineInstrBuilder MIB =
      BuildMI(*BB, MI, DL, TII->get(UsingMaxis32 ? Maxis::LH : Maxis::LH64), Rt);
  for (unsigned i = 1; i < MI.getNumOperands(); i++)
    MIB.add(MI.getOperand(i));

  if(!UsingMaxis32) {
    unsigned Tmp = RegInfo.createVirtualRegister(&Maxis::GPR32RegClass);
    BuildMI(*BB, MI, DL, TII->get(Maxis::COPY), Tmp).addReg(Rt, 0, Maxis::sub_32);
    Rt = Tmp;
  }

  BuildMI(*BB, MI, DL, TII->get(Maxis::FILL_H), Wd).addReg(Rt);
...
MachineBasicBlock *
MaxisSETargetLowering::emitFPROUND_PSEUDO(MachineInstr &MI,
  ...
  bool IsFGR64onMaxis32 = !Subtarget.hasMaxis64() && IsFGR64;
  ...
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  unsigned Wtemp = RegInfo.createVirtualRegister(&Maxis::MSA128WRegClass);
  const TargetRegisterClass *GPRRC =
      IsFGR64onMaxis64 ? &Maxis::GPR64RegClass : &Maxis::GPR32RegClass;
  unsigned MFC1Opc = IsFGR64onMaxis64
                         ? Maxis::DMFC1
                         : (IsFGR64onMaxis32 ? Maxis::MFC1_D64 : Maxis::MFC1);
  unsigned FILLOpc = IsFGR64onMaxis64 ? Maxis::FILL_D : Maxis::FILL_W;
  ...
  if (IsFGR64onMaxis32) {
    ...
    BuildMI(*BB, MI, DL, TII->get(Maxis::MFHC1_D64), Rtemp2).addReg(Fs);
    unsigned Wtemp2 = RegInfo.createVirtualRegister(&Maxis::MSA128WRegClass);
    unsigned Wtemp3 = RegInfo.createVirtualRegister(&Maxis::MSA128WRegClass);
    BuildMI(*BB, MI, DL, TII->get(Maxis::INSERT_W), Wtemp2)
        .addReg(Wtemp)
        .addReg(Rtemp2)
        .addImm(1);
    BuildMI(*BB, MI, DL, TII->get(Maxis::INSERT_W), Wtemp3)
        .addReg(Wtemp2)
        .addReg(Rtemp2)
        .addImm(3);
    WPHI = Wtemp3;
  ...
  if (IsFGR64) {
    unsigned Wtemp2 = RegInfo.createVirtualRegister(&Maxis::MSA128WRegClass);
    BuildMI(*BB, MI, DL, TII->get(Maxis::FEXDO_W), Wtemp2)
        .addReg(WPHI)
        .addReg(WPHI);
    WPHI = Wtemp2;
  }
...
MachineBasicBlock *
MaxisSETargetLowering::emitFPEXTEND_PSEUDO(MachineInstr &MI,
                                          MachineBasicBlock *BB,
                                          bool IsFGR64) const {
  ...
  const TargetRegisterClass *GPRRC =
      IsFGR64onMaxis64 ? &Maxis::GPR64RegClass : &Maxis::GPR32RegClass;
  unsigned MTC1Opc = IsFGR64onMaxis64
                         ? Maxis::DMTC1
                         : (IsFGR64onMaxis32 ? Maxis::MTC1_D64 : Maxis::MTC1);
  unsigned COPYOpc = IsFGR64onMaxis64 ? Maxis::COPY_S_D : Maxis::COPY_S_W;

  unsigned Wtemp = RegInfo.createVirtualRegister(&Maxis::MSA128WRegClass);
  unsigned WPHI = Wtemp;

  BuildMI(*BB, MI, DL, TII->get(Maxis::FEXUPR_W), Wtemp).addReg(Ws);
  if (IsFGR64) {
    WPHI = RegInfo.createVirtualRegister(&Maxis::MSA128DRegClass);
    BuildMI(*BB, MI, DL, TII->get(Maxis::FEXUPR_D), WPHI).addReg(Wtemp);
  }

  // Perform the safety regclass copy mentioned above.
  unsigned Rtemp = RegInfo.createVirtualRegister(GPRRC);
  unsigned FPRPHI = IsFGR64onMaxis32
                        ? RegInfo.createVirtualRegister(&Maxis::FGR64RegClass)
                        : Fd;
  ...
  if (IsFGR64onMaxis32) {
    unsigned Rtemp2 = RegInfo.createVirtualRegister(GPRRC);
    BuildMI(*BB, MI, DL, TII->get(Maxis::COPY_S_W), Rtemp2)
        .addReg(WPHI)
        .addImm(1);
    BuildMI(*BB, MI, DL, TII->get(Maxis::MTHC1_D64), Fd)
        .addReg(FPRPHI)
        .addReg(Rtemp2);
  }
...
// fexp2.w $wd, $ws, $wt
MachineBasicBlock *
MaxisSETargetLowering::emitFEXP2_W_1(MachineInstr &MI,
                                    MachineBasicBlock *BB) const {
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  const TargetRegisterClass *RC = &Maxis::MSA128WRegClass;
  unsigned Ws1 = RegInfo.createVirtualRegister(RC);
  unsigned Ws2 = RegInfo.createVirtualRegister(RC);
  DebugLoc DL = MI.getDebugLoc();

  // Splat 1.0 into a vector
  BuildMI(*BB, MI, DL, TII->get(Maxis::LDI_W), Ws1).addImm(1);
  BuildMI(*BB, MI, DL, TII->get(Maxis::FFINT_U_W), Ws2).addReg(Ws1);

  // Emit 1.0 * fexp2(Wt)
  BuildMI(*BB, MI, DL, TII->get(Maxis::FEXP2_W), MI.getOperand(0).getReg())
      .addReg(Ws2)
      .addReg(MI.getOperand(1).getReg());
...
// fexp2.d $wd, $ws, $wt
MachineBasicBlock *
MaxisSETargetLowering::emitFEXP2_D_1(MachineInstr &MI,
                                    MachineBasicBlock *BB) const {
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  const TargetRegisterClass *RC = &Maxis::MSA128DRegClass;
  unsigned Ws1 = RegInfo.createVirtualRegister(RC);
  unsigned Ws2 = RegInfo.createVirtualRegister(RC);
  DebugLoc DL = MI.getDebugLoc();

  // Splat 1.0 into a vector
  BuildMI(*BB, MI, DL, TII->get(Maxis::LDI_D), Ws1).addImm(1);
  BuildMI(*BB, MI, DL, TII->get(Maxis::FFINT_U_D), Ws2).addReg(Ws1);

  // Emit 1.0 * fexp2(Wt)
  BuildMI(*BB, MI, DL, TII->get(Maxis::FEXP2_D), MI.getOperand(0).getReg())
      .addReg(Ws2)
      .addReg(MI.getOperand(1).getReg());
