//===- MaxisDelaySlotFiller.cpp - Maxis Delay Slot Filler -------------------===//
//
...
static cl::opt<bool> DisableDelaySlotFiller(
  "disable-maxis-delay-filler",
  cl::init(false),
  cl::desc("Fill all delay slots with NOPs."),
  cl::Hidden);

static cl::opt<bool> DisableForwardSearch(
  "disable-maxis-df-forward-search",
  cl::init(true),
  cl::desc("Disallow MAXIS delay filler to search forward."),
  cl::Hidden);

static cl::opt<bool> DisableSuccBBSearch(
  "disable-maxis-df-succbb-search",
  cl::init(true),
  cl::desc("Disallow MAXIS delay filler to search successor basic blocks."),
  cl::Hidden);

static cl::opt<bool> DisableBackwardSearch(
  "disable-maxis-df-backward-search",
  cl::init(false),
  cl::desc("Disallow MAXIS delay filler to search backward."),
  cl::Hidden);
...
static cl::opt<CompactBranchPolicy> MaxisCompactBranchPolicy(
  "maxis-compact-branches",cl::Optional,
  cl::init(CB_Optimal),
  cl::desc("MAXIS Specific: Compact branch policy."),
  cl::values(
...
namespace {
  ...
  public:
    ...
    StringRef getPassName() const override { return "Maxis Delay Slot Filler"; }
  ...
  private:
    ...
    std::pair<MaxisInstrInfo::BranchType, MachineInstr *>
...
void RegDefsUses::setCallerSaved(const MachineInstr &MI) {
  ...
  if (MI.definesRegister(Maxis::RA) || MI.definesRegister(Maxis::RA_64)) {
    Defs.set(Maxis::RA);
    Defs.set(Maxis::RA_64);
  }

  // If MI is a call, add all caller-saved registers to Defs.
  BitVector CallerSavedRegs(TRI.getNumRegs(), true);

  CallerSavedRegs.reset(Maxis::ZERO);
  CallerSavedRegs.reset(Maxis::ZERO_64);
...
void RegDefsUses::setUnallocatableRegs(const MachineFunction &MF) {
  ...
  AllocSet.set(Maxis::ZERO);
  AllocSet.set(Maxis::ZERO_64);
...
Iter Filler::replaceWithCompactBranch(MachineBasicBlock &MBB, Iter Branch,
                                      const DebugLoc &DL) {
  const MaxisSubtarget &STI = MBB.getParent()->getSubtarget<MaxisSubtarget>();
...
static int getEquivalentCallShort(int Opcode) {
  switch (Opcode) {
  case Maxis::BGEZAL:
    return Maxis::BGEZALS_MM;
  case Maxis::BLTZAL:
    return Maxis::BLTZALS_MM;
  case Maxis::JAL:
    return Maxis::JALS_MM;
  case Maxis::JALR:
    return Maxis::JALRS_MM;
  case Maxis::JALR16_MM:
    return Maxis::JALRS16_MM;
  case Maxis::TAILCALL_MM:
    llvm_unreachable("Attempting to shorten the TAILCALL_MM pseudo!");
  case Maxis::TAILCALLREG:
    return Maxis::JR16_MM;
  default:
    llvm_unreachable("Unexpected call instruction for microMAXIS.");
  }
}
...
bool Filler::runOnMachineBasicBlock(MachineBasicBlock &MBB) {
  bool Changed = false;
  const MaxisSubtarget &STI = MBB.getParent()->getSubtarget<MaxisSubtarget>();
  bool InMicroMaxisMode = STI.inMicroMaxisMode();
  const MaxisInstrInfo *TII = STI.getInstrInfo();
  ...
  for (Iter I = MBB.begin(); I != MBB.end(); ++I) {
    ...
    // Delay slot filling is disabled at -O0, or in microMAXIS32R6.
    if (!DisableDelaySlotFiller && (TM->getOptLevel() != CodeGenOpt::None) &&
        !(InMicroMaxisMode && STI.hasMaxis32r6())) {

      bool Filled = false;

      if (MaxisCompactBranchPolicy.getValue() != CB_Always ||
      ...
      if (Filled) {
        ...
        if (InMicroMaxisMode && TII->getInstSizeInBytes(*std::next(DSI)) == 2 &&
            DSI->isCall()) {
    ...
    // For microMAXIS if instruction is BEQ or BNE with one ZERO register, then
    ...
    // For MAXISR6 attempt to produce the corresponding compact (no delay slot)
    ...
    if ((InMicroMaxisMode ||
         (STI.hasMaxis32r6() && MaxisCompactBranchPolicy != CB_Never)) &&
        TII->getEquivalentCompactForm(I)) {
    ...
    BuildMI(MBB, std::next(I), I->getDebugLoc(), TII->get(Maxis::NOP));
...
Filler::getBranch(MachineBasicBlock &MBB, const MachineBasicBlock &Dst) const {
  const MaxisInstrInfo *TII =
      MBB.getParent()->getSubtarget<MaxisSubtarget>().getInstrInfo();
  MachineBasicBlock *TrueBB = nullptr, *FalseBB = nullptr;
  SmallVector<MachineInstr*, 2> BranchInstrs;
  SmallVector<MachineOperand, 2> Cond;

  MaxisInstrInfo::BranchType R =
      TII->analyzeBranch(MBB, TrueBB, FalseBB, Cond, false, BranchInstrs);

  if ((R == MaxisInstrInfo::BT_None) || (R == MaxisInstrInfo::BT_NoBranch))
    return std::make_pair(R, nullptr);

  if (R != MaxisInstrInfo::BT_CondUncond) {
    if (!hasUnoccupiedSlot(BranchInstrs[0]))
      return std::make_pair(MaxisInstrInfo::BT_None, nullptr);

    assert(((R != MaxisInstrInfo::BT_Uncond) || (TrueBB == &Dst)));
  ...
  if (hasUnoccupiedSlot(BranchInstrs[0]))
    return std::make_pair(MaxisInstrInfo::BT_Cond, BranchInstrs[0]);

  // If that fails, try the unconditional branch.
  if (hasUnoccupiedSlot(BranchInstrs[1]) && (FalseBB == &Dst))
    return std::make_pair(MaxisInstrInfo::BT_Uncond, BranchInstrs[1]);
...
bool Filler::examinePred(MachineBasicBlock &Pred, const MachineBasicBlock &Succ,
                         RegDefsUses &RegDU, bool &HasMultipleSuccs,
                         BB2BrMap &BrMap) const {
  std::pair<MaxisInstrInfo::BranchType, MachineInstr *> P =
    getBranch(Pred, Succ);

  // Return if either getBranch wasn't able to analyze the branches or there
  // were no branches with unoccupied slots.
  if (P.first == MaxisInstrInfo::BT_None)
    return false;

  if ((P.first != MaxisInstrInfo::BT_Uncond) &&
      (P.first != MaxisInstrInfo::BT_NoBranch)) {
...
/// createMaxisDelaySlotFillerPass - Returns a pass that fills in delay
/// slots in Maxis MachineFunctions
FunctionPass *llvm::createMaxisDelaySlotFillerPass() { return new Filler(); }
