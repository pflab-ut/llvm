//===- MaxisInstrInfo.h - Maxis Instruction Information -----------*- C++ -*-===//
//
...
// order for MaxisLongBranch pass to work correctly when the code has inline
// assembly.  The returned value doesn't have to be the asm instruction's exact
// size in bytes; MaxisLongBranch only expects it to be the correct upper bound.
//===----------------------------------------------------------------------===//

#ifndef LLVM_LIB_TARGET_MAXIS_MAXISINSTRINFO_H
#define LLVM_LIB_TARGET_MAXIS_MAXISINSTRINFO_H
...
class MaxisInstrInfo : public MaxisGenInstrInfo {
  virtual void anchor();
...
public:
  ...
  explicit MaxisInstrInfo(const MaxisSubtarget &STI, unsigned UncondBrOpc);

  static const MaxisInstrInfo *create(MaxisSubtarget &STI);
  ...
  virtual const MaxisRegisterInfo &getRegisterInfo() const = 0;
...
/// Create MaxisInstrInfo objects.
const MaxisInstrInfo *createMaxis16InstrInfo(const MaxisSubtarget &STI);
const MaxisInstrInfo *createMaxisSEInstrInfo(const MaxisSubtarget &STI);

} // end namespace llvm

#endif // LLVM_LIB_TARGET_MAXIS_MAXISINSTRINFO_H
