//===-- MaxisMCExpr.cpp - Maxis specific MC expression classes --------------===//
//
...
#define DEBUG_TYPE "maxismcexpr"

const MaxisMCExpr *MaxisMCExpr::create(MaxisMCExpr::MaxisExprKind Kind,
                                     const MCExpr *Expr, MCContext &Ctx) {
  return new (Ctx) MaxisMCExpr(Kind, Expr);
}

const MaxisMCExpr *MaxisMCExpr::createGpOff(MaxisMCExpr::MaxisExprKind Kind,
                                          const MCExpr *Expr, MCContext &Ctx) {
  return create(Kind, create(MEK_NEG, create(MEK_GPREL, Expr, Ctx), Ctx), Ctx);
}

void MaxisMCExpr::printImpl(raw_ostream &OS, const MCAsmInfo *MAI) const {
  int64_t AbsVal;
...
bool
MaxisMCExpr::evaluateAsRelocatableImpl(MCValue &Res,
                                      const MCAsmLayout *Layout,
                                      const MCFixup *Fixup) const {
  // Look for the %hi(%neg(%gp_rel(X))) and %lo(%neg(%gp_rel(X))) special cases.
  if (isGpOff()) {
    const MCExpr *SubExpr =
        cast<MaxisMCExpr>(cast<MaxisMCExpr>(getSubExpr())->getSubExpr())
            ->getSubExpr();
...
void MaxisMCExpr::visitUsedExpr(MCStreamer &Streamer) const {
...
static void fixELFSymbolsInTLSFixupsImpl(const MCExpr *Expr, MCAssembler &Asm) {
  switch (Expr->getKind()) {
  case MCExpr::Target:
    fixELFSymbolsInTLSFixupsImpl(cast<MaxisMCExpr>(Expr)->getSubExpr(), Asm);
    break;
...
void MaxisMCExpr::fixELFSymbolsInTLSFixups(MCAssembler &Asm) const {
  switch (getKind()) {
  case MEK_None:
  case MEK_Special:
    llvm_unreachable("MEK_None and MEK_Special are invalid");
  ...
  case MEK_PCREL_LO16:
    ...
    // a consecutive chain.
    if (const MaxisMCExpr *E = dyn_cast<const MaxisMCExpr>(getSubExpr()))
...
bool MaxisMCExpr::isGpOff(MaxisExprKind &Kind) const {
  if (getKind() == MEK_HI || getKind() == MEK_LO) {
    if (const MaxisMCExpr *S1 = dyn_cast<const MaxisMCExpr>(getSubExpr())) {
      if (const MaxisMCExpr *S2 = dyn_cast<const MaxisMCExpr>(S1->getSubExpr())) {
