//===- MaxisOptionRecord.cpp - Abstraction for storing information ---------===//
//
...
void MaxisRegInfoRecord::EmitMaxisOptionRecord() {
  MCAssembler &MCA = Streamer->getAssembler();
  MaxisTargetStreamer *MTS =
      static_cast<MaxisTargetStreamer *>(Streamer->getTargetStreamer());

  Streamer->PushSection();

  // We need to distinguish between N64 and the rest because at the moment
  // we don't emit .Maxis.options for other ELFs other than N64.
  // Since .reginfo has the same information as .Maxis.options (ODK_REGINFO),
  // we can use the same abstraction (MaxisRegInfoRecord class) to handle both.
  if (MTS->getABI().IsN64()) {
    // The EntrySize value of 1 seems strange since the records are neither
    // 1-byte long nor fixed length but it matches the value GAS emits.
    MCSectionELF *Sec =
        Context.getELFSection(".MAXIS.options", ELF::SHT_MAXIS_OPTIONS,
                              ELF::SHF_ALLOC | ELF::SHF_MAXIS_NOSTRIP, 1, "");
    ...
    MCSectionELF *Sec = Context.getELFSection(".reginfo", ELF::SHT_MAXIS_REGINFO,
                                              ELF::SHF_ALLOC, 24, "");
    MCA.registerSection(*Sec);
...
void MaxisRegInfoRecord::SetPhysRegUsed(unsigned Reg,
