//===- MaxisDisassembler.cpp - Disassembler for Maxis -----------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file is part of the Maxis Disassembler.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "maxis-disassembler"

using DecodeStatus = MCDisassembler::DecodeStatus;

namespace {

class MaxisDisassembler : public MCDisassembler {
  bool IsMicroMaxis;
  bool IsBigEndian;

public:
  MaxisDisassembler(const MCSubtargetInfo &STI, MCContext &Ctx, bool IsBigEndian)
      : MCDisassembler(STI, Ctx),
        IsMicroMaxis(STI.getFeatureBits()[Maxis::FeatureMicroMaxis]),
        IsBigEndian(IsBigEndian) {}

  bool hasMaxis2() const { return STI.getFeatureBits()[Maxis::FeatureMaxis2]; }
  bool hasMaxis3() const { return STI.getFeatureBits()[Maxis::FeatureMaxis3]; }
  bool hasMaxis32() const { return STI.getFeatureBits()[Maxis::FeatureMaxis32]; }

  bool hasMaxis32r6() const {
    return STI.getFeatureBits()[Maxis::FeatureMaxis32r6];
  }

  bool isFP64() const { return STI.getFeatureBits()[Maxis::FeatureFP64Bit]; }

  bool isGP64() const { return STI.getFeatureBits()[Maxis::FeatureGP64Bit]; }

  bool isPTR64() const { return STI.getFeatureBits()[Maxis::FeaturePTR64Bit]; }

  bool hasCnMaxis() const { return STI.getFeatureBits()[Maxis::FeatureCnMaxis]; }

  bool hasCOP3() const {
    // Only present in MAXIS-I and MAXIS-II
    return !hasMaxis32() && !hasMaxis3();
  }
...
// DecodeBranchTarget7MM - Decode microMAXIS branch offset, which is
...
// DecodeBranchTarget10MM - Decode microMAXIS branch offset, which is
// shifted left by 1 bit.
...
// DecodeBranchTargetMM - Decode microMAXIS branch offset, which is
// shifted left by 1 bit.
...
// DecodeBranchTarget26MM - Decode microMAXIS branch offset, which is
// shifted left by 1 bit.
...
// DecodeJumpTargetMM - Decode microMAXIS jump target, which is
// shifted left by 1 bit.
...
static MCDisassembler *createMaxisDisassembler(
                       const Target &T,
                       const MCSubtargetInfo &STI,
                       MCContext &Ctx) {
  return new MaxisDisassembler(STI, Ctx, true);
}

static MCDisassembler *createMaxiselDisassembler(
                       const Target &T,
                       const MCSubtargetInfo &STI,
                       MCContext &Ctx) {
  return new MaxisDisassembler(STI, Ctx, false);
}

extern "C" void LLVMInitializeMaxisDisassembler() {
  // Register the disassembler.
  TargetRegistry::RegisterMCDisassembler(getTheMaxisTarget(),
                                         createMaxisDisassembler);
  TargetRegistry::RegisterMCDisassembler(getTheMaxiselTarget(),
                                         createMaxiselDisassembler);
  TargetRegistry::RegisterMCDisassembler(getTheMaxis64Target(),
                                         createMaxisDisassembler);
  TargetRegistry::RegisterMCDisassembler(getTheMaxis64elTarget(),
                                         createMaxiselDisassembler);
}

#include "MaxisGenDisassemblerTables.inc"

static unsigned getReg(const void *D, unsigned RC, unsigned RegNo) {
  const MaxisDisassembler *Dis = static_cast<const MaxisDisassembler*>(D);
...
static DecodeStatus DecodeDAHIDATIMMR6(MCInst &MI, InsnType insn, uint64_t Address,
  ...
  MI.addOperand(MCOperand::createReg(getReg(Decoder, Maxis::GPR64RegClassID,
                                       Rs)));
  MI.addOperand(MCOperand::createReg(getReg(Decoder, Maxis::GPR64RegClassID,
                                       Rs)));
...
static DecodeStatus DecodeAddiGroupBranch(MCInst &MI, InsnType insn,
                                          uint64_t Address,
                                          const void *Decoder) {
  // If we are called then we can assume that MAXIS32r6/MAXIS64r6 is enabled
  ...
  if (Rs >= Rt) {
    MI.setOpcode(Maxis::BOVC);
    HasRs = true;
  } else if (Rs != 0 && Rs < Rt) {
    MI.setOpcode(Maxis::BEQC);
    HasRs = true;
  } else
    MI.setOpcode(Maxis::BEQZALC);

  if (HasRs)
    MI.addOperand(MCOperand::createReg(getReg(Decoder, Maxis::GPR32RegClassID,
                                       Rs)));
...
static DecodeStatus DecodePOP35GroupBranchMMR6(MCInst &MI, InsnType insn,
  ...
  if (Rs >= Rt) {
    ...
    MI.setOpcode(Maxis::BEQZALC_MMR6);
    MI.addOperand(MCOperand::createReg(getReg(Decoder, Maxis::GPR32RegClassID,
                                       Rt)));
    Imm = SignExtend64(fieldFromInstruction(insn, 0, 16), 16) * 2 + 4;
  }
...
static DecodeStatus DecodeDaddiGroupBranch(MCInst &MI, InsnType insn,
  ...
  if (Rs >= Rt) {
    MI.setOpcode(Maxis::BNVC);
    HasRs = true;
  } else if (Rs != 0 && Rs < Rt) {
    MI.setOpcode(Maxis::BNEC);
    HasRs = true;
  } else
    MI.setOpcode(Maxis::BNEZALC);
...
static DecodeStatus DecodePOP37GroupBranchMMR6(MCInst &MI, InsnType insn,
  ...
  if (Rs >= Rt) {
    ...
    MI.setOpcode(Maxis::BNEZALC_MMR6);
    MI.addOperand(MCOperand::createReg(getReg(Decoder, Maxis::GPR32RegClassID,
                                       Rt)));
    Imm = SignExtend64(fieldFromInstruction(insn, 0, 16), 16) * 2 + 4;
  }
...
static DecodeStatus DecodePOP65GroupBranchMMR6(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0)
    ...
    MI.setOpcode(Maxis::BGTZC_MMR6);
  else if (Rs == Rt)
    MI.setOpcode(Maxis::BLTZC_MMR6);
  else {
    MI.setOpcode(Maxis::BLTC_MMR6);
    HasRs = true;
  }
...
static DecodeStatus DecodePOP75GroupBranchMMR6(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0)
    ...
    MI.setOpcode(Maxis::BLEZC_MMR6);
  else if (Rs == Rt)
    MI.setOpcode(Maxis::BGEZC_MMR6);
  else {
    HasRs = true;
    MI.setOpcode(Maxis::BGEC_MMR6);
  }
...
static DecodeStatus DecodeBlezlGroupBranch(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0)
    ...
    MI.setOpcode(Maxis::BLEZC);
  else if (Rs == Rt)
    MI.setOpcode(Maxis::BGEZC);
  else {
    HasRs = true;
    MI.setOpcode(Maxis::BGEC);
  }
...
static DecodeStatus DecodeBgtzlGroupBranch(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0)
    ...
    MI.setOpcode(Maxis::BGTZC);
  else if (Rs == Rt)
    MI.setOpcode(Maxis::BLTZC);
  else {
    MI.setOpcode(Maxis::BLTC);
    HasRs = true;
  }
...
static DecodeStatus DecodeBgtzGroupBranch(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0) {
    ...
    MI.setOpcode(Maxis::BLTZALC);
    HasRs = true;
  } else {
    MI.setOpcode(Maxis::BLTUC);
    HasRs = true;
    HasRt = true;
  }
...
static DecodeStatus DecodeBlezGroupBranch(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0)
    ...
    MI.setOpcode(Maxis::BLEZALC);
  else if (Rs == Rt)
    MI.setOpcode(Maxis::BGEZALC);
  else {
    HasRs = true;
    MI.setOpcode(Maxis::BGEUC);
  }
...
static DecodeStatus DecodeDEXT(MCInst &MI, InsnType Insn, uint64_t Address,
  ...
  MI.setOpcode(Maxis::DEXT);

  InsnType Rs = fieldFromInstruction(Insn, 21, 5);
  InsnType Rt = fieldFromInstruction(Insn, 16, 5);

  MI.addOperand(MCOperand::createReg(getReg(Decoder, Maxis::GPR64RegClassID, Rt)));
  MI.addOperand(MCOperand::createReg(getReg(Decoder, Maxis::GPR64RegClassID, Rs)));
...
static DecodeStatus readInstruction32(ArrayRef<uint8_t> Bytes, uint64_t Address,
                                      uint64_t &Size, uint32_t &Insn,
                                      bool IsBigEndian, bool IsMicroMaxis) {
  ...
  // High 16 bits of a 32-bit microMAXIS instruction (where the opcode is)
  // always precede the low 16 bits in the instruction stream (that is, they
  // are placed at lower addresses in the instruction stream).
  //
  // microMAXIS byte ordering:
  //   Big-endian:    0 | 1 | 2 | 3
...
DecodeStatus MaxisDisassembler::getInstruction(MCInst &Instr, uint64_t &Size,
  ...
  if (IsMicroMaxis) {
    ...
    if (hasMaxis32r6()) {
      DEBUG(dbgs() << "Trying MicroMaxisR616 table (16-bit instructions):\n");
      // Calling the auto-generated decoder function for microMAXIS32R6
      // 16-bit instructions.
      Result = decodeInstruction(DecoderTableMicroMaxisR616, Instr, Insn,
                                 Address, this, STI);
      if (Result != MCDisassembler::Fail) {
        Size = 2;
        return Result;
    ...
    DEBUG(dbgs() << "Trying MicroMaxis16 table (16-bit instructions):\n");
    // Calling the auto-generated decoder function for microMAXIS 16-bit
    // instructions.
    Result = decodeInstruction(DecoderTableMicroMaxis16, Instr, Insn, Address,
                               this, STI);
    if (Result != MCDisassembler::Fail) {
      Size = 2;
      return Result;
    ...
    if (hasMaxis32r6()) {
      DEBUG(dbgs() << "Trying MicroMaxis32r632 table (32-bit instructions):\n");
      // Calling the auto-generated decoder function.
      Result = decodeInstruction(DecoderTableMicroMaxisR632, Instr, Insn, Address,
                                 this, STI);
      if (Result != MCDisassembler::Fail) {
        Size = 4;
        return Result;
    ...
    DEBUG(dbgs() << "Trying MicroMaxis32 table (32-bit instructions):\n");
    // Calling the auto-generated decoder function.
    Result = decodeInstruction(DecoderTableMicroMaxis32, Instr, Insn, Address,
                               this, STI);
    if (Result != MCDisassembler::Fail) {
      Size = 4;
      return Result;
    ...
    if (isFP64()) {
      DEBUG(dbgs() << "Trying MicroMaxisFP64 table (32-bit opcodes):\n");
      Result = decodeInstruction(DecoderTableMicroMaxisFP6432, Instr, Insn,
                                 Address, this, STI);
      if (Result != MCDisassembler::Fail) {
        Size = 4;
        return Result;
    ...
    // microMAXIS instructions have a minimum alignment of 2, the next 2 bytes
  ...
  // The only instruction size for standard encoded MAXIS.
  Size = 4;
  ...
  if (hasMaxis32r6() && isGP64()) {
    DEBUG(dbgs() << "Trying Maxis32r6_64r6 (GPR64) table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableMaxis32r6_64r6_GP6432, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }

  if (hasMaxis32r6() && isPTR64()) {
    DEBUG(dbgs() << "Trying Maxis32r6_64r6 (PTR64) table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableMaxis32r6_64r6_PTR6432, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }
  ...
  if (hasMaxis32r6()) {
    DEBUG(dbgs() << "Trying Maxis32r6_64r6 table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableMaxis32r6_64r632, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }

  if (hasMaxis2() && isPTR64()) {
    DEBUG(dbgs() << "Trying Maxis32r6_64r6 (PTR64) table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableMaxis32_64_PTR6432, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }
  ...
  if (hasCnMaxis()) {
    DEBUG(dbgs() << "Trying CnMaxis table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableCnMaxis32, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }
  ...
  if (isGP64()) {
    DEBUG(dbgs() << "Trying Maxis64 (GPR64) table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableMaxis6432, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }
  ...
  if (isFP64()) {
    DEBUG(dbgs() << "Trying MaxisFP64 (64 bit FPU) table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableMaxisFP6432, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }

  DEBUG(dbgs() << "Trying Maxis table (32-bit opcodes):\n");
  // Calling the auto-generated decoder function.
  Result =
      decodeInstruction(DecoderTableMaxis32, Instr, Insn, Address, this, STI);
  if (Result != MCDisassembler::Fail)
    return Result;
...
static DecodeStatus DecodeGPR64RegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Maxis::GPR64RegClassID, RegNo);
...
static DecodeStatus DecodeGPRMM16RegisterClass(MCInst &Inst,
  ...
  if (RegNo > 7)
    return MCDisassembler::Fail;
  unsigned Reg = getReg(Decoder, Maxis::GPRMM16RegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeGPRMM16ZeroRegisterClass(MCInst &Inst,
  ...
  if (RegNo > 7)
    return MCDisassembler::Fail;
  unsigned Reg = getReg(Decoder, Maxis::GPRMM16ZeroRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeGPRMM16MovePRegisterClass(MCInst &Inst,
  ...
  if (RegNo > 7)
    return MCDisassembler::Fail;
  unsigned Reg = getReg(Decoder, Maxis::GPRMM16MovePRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeGPR32RegisterClass(MCInst &Inst,
  ...
  if (RegNo > 31)
    return MCDisassembler::Fail;
  unsigned Reg = getReg(Decoder, Maxis::GPR32RegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodePtrRegisterClass(MCInst &Inst,
                                           ...
                                           uint64_t Address,
                                           const void *Decoder) {
  if (static_cast<const MaxisDisassembler *>(Decoder)->isGP64())
...
static DecodeStatus DecodeFGR64RegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Maxis::FGR64RegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeFGR32RegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Maxis::FGR32RegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeCCRRegisterClass(MCInst &Inst,
  ...
  if (RegNo > 31)
    return MCDisassembler::Fail;
  unsigned Reg = getReg(Decoder, Maxis::CCRRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeFCCRegisterClass(MCInst &Inst,
  ...
  if (RegNo > 7)
    return MCDisassembler::Fail;
  unsigned Reg = getReg(Decoder, Maxis::FCCRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeFGRCCRegisterClass(MCInst &Inst, unsigned RegNo,
  ...
  unsigned Reg = getReg(Decoder, Maxis::FGRCCRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeMem(MCInst &Inst,
  ...
  Reg = getReg(Decoder, Maxis::GPR32RegClassID, Reg);
  Base = getReg(Decoder, Maxis::GPR32RegClassID, Base);

  if (Inst.getOpcode() == Maxis::SC ||
      Inst.getOpcode() == Maxis::SCD)
    Inst.addOperand(MCOperand::createReg(Reg));
...
static DecodeStatus DecodeMSA128Mem(MCInst &Inst, unsigned Insn,
  ...
  Reg = getReg(Decoder, Maxis::MSA128BRegClassID, Reg);
  Base = getReg(Decoder, Maxis::GPR32RegClassID, Base);
...
static DecodeStatus DecodeMemMMImm9(MCInst &Inst,
  ...
  if (Inst.getOpcode() == Maxis::SCE_MM)
    Inst.addOperand(MCOperand::createReg(Reg));
...
static DecodeStatus DecodeMemMMImm12(MCInst &Inst,
  ...
  default:
    Inst.addOperand(MCOperand::createReg(Reg));
    if (Inst.getOpcode() == Maxis::LWP_MM || Inst.getOpcode() == Maxis::SWP_MM ||
        Inst.getOpcode() == Maxis::LWP_MMR6 || Inst.getOpcode() == Maxis::SWP_MMR6)
...
static DecodeStatus DecodeFMem(MCInst &Inst,
  ...
  Reg = getReg(Decoder, Maxis::FGR64RegClassID, Reg);
  Base = getReg(Decoder, Maxis::GPR32RegClassID, Base);
...
static DecodeStatus DecodeFMemMMR2(MCInst &Inst, unsigned Insn,
  ...
  // swapped according to microMAXIS spec.
  int Offset = SignExtend32<16>(Insn & 0xffff);
  unsigned Base = fieldFromInstruction(Insn, 16, 5);
  unsigned Reg = fieldFromInstruction(Insn, 21, 5);
...
static DecodeStatus DecodeFMem2(MCInst &Inst,
  ...
  Reg = getReg(Decoder, Maxis::COP2RegClassID, Reg);
  Base = getReg(Decoder, Maxis::GPR32RegClassID, Base);
...
static DecodeStatus DecodeFMem3(MCInst &Inst,
  ...
  Reg = getReg(Decoder, Maxis::COP3RegClassID, Reg);
  Base = getReg(Decoder, Maxis::GPR32RegClassID, Base);
...
static DecodeStatus DecodeSpecial3LlSc(MCInst &Inst,
  ...
  Rt = getReg(Decoder, Maxis::GPR32RegClassID, Rt);
  Base = getReg(Decoder, Maxis::GPR32RegClassID, Base);

  if(Inst.getOpcode() == Maxis::SC_R6 || Inst.getOpcode() == Maxis::SCD_R6){
...
static DecodeStatus DecodeHWRegsRegisterClass(MCInst &Inst,
  ...
  if (RegNo != 29)
    return  MCDisassembler::Fail;
  Inst.addOperand(MCOperand::createReg(Maxis::HWR29));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeAFGR64RegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Maxis::AFGR64RegClassID, RegNo /2);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeACC64DSPRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Maxis::ACC64DSPRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeHI32DSPRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Maxis::HI32DSPRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeLO32DSPRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Maxis::LO32DSPRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeMSA128BRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Maxis::MSA128BRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeMSA128HRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Maxis::MSA128HRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeMSA128WRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Maxis::MSA128WRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeMSA128DRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Maxis::MSA128DRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeMSACtrlRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Maxis::MSACtrlRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeCOP0RegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Maxis::COP0RegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeCOP2RegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Maxis::COP2RegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeBranchTarget(MCInst &Inst,
  ...
  int32_t BranchOffset = (SignExtend32<16>(Offset) * 4);
...
static DecodeStatus DecodeRegListOperand(MCInst &Inst,
                                         ...
                                         uint64_t Address,
                                         const void *Decoder) {
  unsigned Regs[] = {Maxis::S0, Maxis::S1, Maxis::S2, Maxis::S3, Maxis::S4, Maxis::S5,
                     Maxis::S6, Maxis::S7, Maxis::FP};
  unsigned RegNum;
  ...
  if (RegLst & 0x10)
    Inst.addOperand(MCOperand::createReg(Maxis::RA));
...
static DecodeStatus DecodeRegListOperand16(MCInst &Inst, unsigned Insn,
                                           uint64_t Address,
                                           const void *Decoder) {
  unsigned Regs[] = {Maxis::S0, Maxis::S1, Maxis::S2, Maxis::S3};
  unsigned RegLst;
  switch(Inst.getOpcode()) {
  default:
...
static DecodeStatus DecodeMovePRegPair(MCInst &Inst, unsigned RegPair,
  ...
  default:
    return MCDisassembler::Fail;
  case 0:
    Inst.addOperand(MCOperand::createReg(Maxis::A1));
    Inst.addOperand(MCOperand::createReg(Maxis::A2));
    break;
  case 1:
    Inst.addOperand(MCOperand::createReg(Maxis::A1));
    Inst.addOperand(MCOperand::createReg(Maxis::A3));
    break;
  case 2:
    Inst.addOperand(MCOperand::createReg(Maxis::A2));
    Inst.addOperand(MCOperand::createReg(Maxis::A3));
    break;
  case 3:
    Inst.addOperand(MCOperand::createReg(Maxis::A0));
    Inst.addOperand(MCOperand::createReg(Maxis::S5));
    break;
  case 4:
    Inst.addOperand(MCOperand::createReg(Maxis::A0));
    Inst.addOperand(MCOperand::createReg(Maxis::S6));
    break;
  case 5:
    Inst.addOperand(MCOperand::createReg(Maxis::A0));
    Inst.addOperand(MCOperand::createReg(Maxis::A1));
...
static DecodeStatus DecodeBgtzGroupBranchMMR6(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0)
    ...
    MI.setOpcode(Maxis::BGTZALC_MMR6);
    HasRt = true;
    Imm = SignExtend64(fieldFromInstruction(insn, 0, 16), 16) * 2 + 4;
  }
  else if (Rs == Rt) {
    MI.setOpcode(Maxis::BLTZALC_MMR6);
    HasRs = true;
    Imm = SignExtend64(fieldFromInstruction(insn, 0, 16), 16) * 2 + 4;
  }
  else {
    MI.setOpcode(Maxis::BLTUC_MMR6);
    HasRs = true;
    HasRt = true;
    Imm = SignExtend64(fieldFromInstruction(insn, 0, 16), 16) * 4 + 4;
  }
  ...
  if (HasRs)
    MI.addOperand(
    MCOperand::createReg(getReg(Decoder, Maxis::GPR32RegClassID, Rs)));

  if (HasRt)
    MI.addOperand(
    MCOperand::createReg(getReg(Decoder, Maxis::GPR32RegClassID, Rt)));
...
static DecodeStatus DecodeBlezGroupBranchMMR6(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0)
    ...
    MI.setOpcode(Maxis::BLEZALC_MMR6);
    Imm = SignExtend64(fieldFromInstruction(insn, 0, 16), 16) * 2 + 4;
  }
  else if (Rs == Rt) {
    MI.setOpcode(Maxis::BGEZALC_MMR6);
    Imm = SignExtend64(fieldFromInstruction(insn, 0, 16), 16) * 2 + 4;
  }
  else {
    HasRs = true;
    MI.setOpcode(Maxis::BGEUC_MMR6);
    Imm = SignExtend64(fieldFromInstruction(insn, 0, 16), 16) * 4 + 4;
  }
