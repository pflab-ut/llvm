//===-- MaxisFrameLowering.cpp - Maxis Frame Information --------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the Maxis implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
const MaxisFrameLowering *MaxisFrameLowering::create(const MaxisSubtarget &ST) {
  if (ST.inMaxis16Mode())
    return llvm::createMaxis16FrameLowering(ST);

  return llvm::createMaxisSEFrameLowering(ST);
}
...
bool MaxisFrameLowering::hasFP(const MachineFunction &MF) const {
...
bool MaxisFrameLowering::hasBP(const MachineFunction &MF) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
  const TargetRegisterInfo *TRI = STI.getRegisterInfo();
...
uint64_t MaxisFrameLowering::estimateStackSize(const MachineFunction &MF) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
  const TargetRegisterInfo &TRI = *STI.getRegisterInfo();
...
MachineBasicBlock::iterator MaxisFrameLowering::
eliminateCallFramePseudoInstr(MachineFunction &MF, MachineBasicBlock &MBB,
                              MachineBasicBlock::iterator I) const {
  unsigned SP = STI.getABI().IsN64() ? Maxis::SP_64 : Maxis::SP;

  if (!hasReservedCallFrame(MF)) {
    int64_t Amount = I->getOperand(0).getImm();
    if (I->getOpcode() == Maxis::ADJCALLSTACKDOWN)
      Amount = -Amount;
