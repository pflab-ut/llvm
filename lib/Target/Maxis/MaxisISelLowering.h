//===- MaxisISelLowering.h - Maxis DAG Lowering Interface ---------*- C++ -*-===//
//
...
#ifndef LLVM_LIB_TARGET_MAXIS_MAXISISELLOWERING_H
#define LLVM_LIB_TARGET_MAXIS_MAXISISELLOWERING_H
...
class TargetRegisterClass;
  ...
  namespace MaxisISD {
    ...
    enum NodeType : unsigned {
      ...
      // No relation with Maxis Hi register
      Hi,

      // Get the Lower 16 bits from a 32/64-bit immediate
      // No relation with Maxis Lo register
      Lo,
  ...
  } // ene namespace MaxisISD

  //===--------------------------------------------------------------------===//
  // TargetLowering Implementation
  //===--------------------------------------------------------------------===//

  class MaxisTargetLowering : public TargetLowering  {
    bool isMicroMaxis;

  public:
    explicit MaxisTargetLowering(const MaxisTargetMachine &TM,
                                const MaxisSubtarget &STI);

    static const MaxisTargetLowering *create(const MaxisTargetMachine &TM,
                                            const MaxisSubtarget &STI);
    ...
    getExceptionPointerRegister(const Constant *PersonalityFn) const override {
      return ABI.IsN64() ? Maxis::A0_64 : Maxis::A0;
    }
    ...
    getExceptionSelectorRegister(const Constant *PersonalityFn) const override {
      return ABI.IsN64() ? Maxis::A1_64 : Maxis::A1;
    }

    /// Returns true if a cast between SrcAS and DestAS is a noop.
    bool isNoopAddrSpaceCast(unsigned SrcAS, unsigned DestAS) const override {
      // Maxis doesn't have any special address spaces so we just reserve
  ...
  protected:
    ...
    SDValue getAddrLocal(NodeTy *N, const SDLoc &DL, EVT Ty, SelectionDAG &DAG,
      ...
      unsigned GOTFlag = IsN32OrN64 ? MaxisII::MO_GOT_PAGE : MaxisII::MO_GOT;
      SDValue GOT = DAG.getNode(MaxisISD::Wrapper, DL, Ty, getGlobalReg(DAG, Ty),
                                getTargetNode(N, Ty, DAG, GOTFlag));
      SDValue Load =
          DAG.getLoad(Ty, DL, DAG.getEntryNode(), GOT,
                      MachinePointerInfo::getGOT(DAG.getMachineFunction()));
      unsigned LoFlag = IsN32OrN64 ? MaxisII::MO_GOT_OFST : MaxisII::MO_ABS_LO;
      SDValue Lo = DAG.getNode(MaxisISD::Lo, DL, Ty,
    ...
    SDValue getAddrGlobal(NodeTy *N, const SDLoc &DL, EVT Ty, SelectionDAG &DAG,
      ...
      SDValue Tgt = DAG.getNode(MaxisISD::Wrapper, DL, Ty, getGlobalReg(DAG, Ty),
    ...
    SDValue getAddrGlobalLargeGOT(NodeTy *N, const SDLoc &DL, EVT Ty,
                                  ...
                                  const MachinePointerInfo &PtrInfo) const {
      SDValue Hi = DAG.getNode(MaxisISD::GotHi, DL, Ty,
                               getTargetNode(N, Ty, DAG, HiFlag));
      Hi = DAG.getNode(ISD::ADD, DL, Ty, Hi, getGlobalReg(DAG, Ty));
      SDValue Wrapper = DAG.getNode(MaxisISD::Wrapper, DL, Ty, Hi,
                                    getTargetNode(N, Ty, DAG, LoFlag));
    ...
    SDValue getAddrNonPIC(NodeTy *N, const SDLoc &DL, EVT Ty,
      ...
      SDValue Hi = getTargetNode(N, Ty, DAG, MaxisII::MO_ABS_HI);
      SDValue Lo = getTargetNode(N, Ty, DAG, MaxisII::MO_ABS_LO);
      return DAG.getNode(ISD::ADD, DL, Ty,
                         DAG.getNode(MaxisISD::Hi, DL, Ty, Hi),
                         DAG.getNode(MaxisISD::Lo, DL, Ty, Lo));
   }
   ...
   //            16), %lo(%sym))
   //
   // FIXME: This method is not efficent for (micro)MAXIS64R6.
   template <class NodeTy>
   SDValue getAddrNonPICSym64(NodeTy *N, const SDLoc &DL, EVT Ty,
      ...
      SDValue Highest =
          DAG.getNode(MaxisISD::Highest, DL, Ty,
                      getTargetNode(N, Ty, DAG, MaxisII::MO_HIGHEST));
      SDValue Higher = getTargetNode(N, Ty, DAG, MaxisII::MO_HIGHER);
      SDValue HigherPart =
          DAG.getNode(ISD::ADD, DL, Ty, Highest,
                      DAG.getNode(MaxisISD::Higher, DL, Ty, Higher));
    ...
    SDValue getAddrGPRel(NodeTy *N, const SDLoc &DL, EVT Ty,
      ...
      SDValue GPRel = getTargetNode(N, Ty, DAG, MaxisII::MO_GPREL);
      return DAG.getNode(
          ISD::ADD, DL, Ty,
          DAG.getRegister(IsN64 ? Maxis::GP_64 : Maxis::GP, Ty),
          DAG.getNode(MaxisISD::GPRel, DL, DAG.getVTList(Ty), GPRel));
    }
  ...
  private:
    ...
    isEligibleForTailCallOptimization(const CCState &CCInfo,
                                      ...
                                      const MaxisFunctionInfo &FI) const = 0;
  ...
  /// Create MaxisTargetLowering objects.
  const MaxisTargetLowering *
  createMaxis16TargetLowering(const MaxisTargetMachine &TM,
                             const MaxisSubtarget &STI);
  const MaxisTargetLowering *
  createMaxisSETargetLowering(const MaxisTargetMachine &TM,
                             const MaxisSubtarget &STI);
...
#endif // LLVM_LIB_TARGET_MAXIS_MAXISISELLOWERING_H
