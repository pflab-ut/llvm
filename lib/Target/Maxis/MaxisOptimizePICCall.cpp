//===- MaxisOptimizePICCall.cpp - Optimize PIC Calls -----------------------===//
//
...
#define DEBUG_TYPE "optimize-maxis-pic-call"

static cl::opt<bool> LoadTargetFromGOT("maxis-load-target-from-got",
                                       cl::init(true),
                                       cl::desc("Load target address from GOT"),
                                       cl::Hidden);

static cl::opt<bool> EraseGPOpnd("maxis-erase-gp-opnd",
...
public:
  ...
  StringRef getPassName() const override { return "Maxis OptimizePICCall"; }
...
static void setCallTargetReg(MachineBasicBlock *MBB,
  ...
  unsigned DstReg = getRegTy(SrcReg, MF) == MVT::i32 ? Maxis::T9 : Maxis::T9_64;
...
static void eraseGPOpnd(MachineInstr &MI) {
  ...
  unsigned Reg = Ty == MVT::i32 ? Maxis::GP : Maxis::GP_64;
...
bool OptimizePICCall::runOnMachineFunction(MachineFunction &F) {
  if (static_cast<const MaxisSubtarget &>(F.getSubtarget()).inMaxis16Mode())
    return false;
...
bool OptimizePICCall::isCallViaRegister(MachineInstr &MI, unsigned &Reg,
  ...
  if (Flags != MaxisII::MO_GOT_CALL && Flags != MaxisII::MO_CALL_LO16)
    return true;
...
FunctionPass *llvm::createMaxisOptimizePICCallPass() {
