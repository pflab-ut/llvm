//===---- MaxisCCState.cpp - CCState with Maxis specific extensions ---------===//
//
...
MaxisCCState::SpecialCallingConvType
MaxisCCState::getSpecialCallingConvForCallee(const SDNode *Callee,
                                            const MaxisSubtarget &Subtarget) {
  MaxisCCState::SpecialCallingConvType SpecialCallingConv = NoSpecialCallingConv;
  if (Subtarget.inMaxis16HardFloat()) {
    if (const GlobalAddressSDNode *G =
      ...
      if (F && F->hasFnAttribute("__Maxis16RetHelper")) {
        SpecialCallingConv = Maxis16RetHelperConv;
      }
    }
  }
...
void MaxisCCState::PreAnalyzeCallResultForF128(
...
/// record this for use by RetCC_MaxisN.
void MaxisCCState::PreAnalyzeReturnForF128(
...
/// this.
void MaxisCCState::PreAnalyzeCallResultForVectorFloat(
...
/// this.
void MaxisCCState::PreAnalyzeReturnForVectorFloat(
    const SmallVectorImpl<ISD::OutputArg> &Outs) {
  for (unsigned i = 0; i < Outs.size(); ++i) {
...
void MaxisCCState::PreAnalyzeCallOperands(
    const SmallVectorImpl<ISD::OutputArg> &Outs,
...
/// and record this.
void MaxisCCState::PreAnalyzeFormalArgumentsForF128(
    const SmallVectorImpl<ISD::InputArg> &Ins) {
  const MachineFunction &MF = getMachineFunction();
  for (unsigned i = 0; i < Ins.size(); ++i) {
    ...
    // The MAXIS vector ABI exhibits a corner case of sorts or quirk; if the
