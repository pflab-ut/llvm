//===-- MaxisTargetObjectFile.cpp - Maxis Object Files ----------------------===//
//
...
SSThreshold("maxis-ssection-threshold", cl::Hidden,
...
void MaxisTargetObjectFile::Initialize(MCContext &Ctx, const TargetMachine &TM){
  TargetLoweringObjectFileELF::Initialize(Ctx, TM);
  InitializeELF(TM.Options.UseInitArray);

  SmallDataSection = getContext().getELFSection(
      ".sdata", ELF::SHT_PROGBITS,
      ELF::SHF_WRITE | ELF::SHF_ALLOC | ELF::SHF_MAXIS_GPREL);

  SmallBSSSection = getContext().getELFSection(".sbss", ELF::SHT_NOBITS,
                                               ELF::SHF_WRITE | ELF::SHF_ALLOC |
                                                   ELF::SHF_MAXIS_GPREL);
  this->TM = &static_cast<const MaxisTargetMachine &>(TM);
}
...
/// section.
bool MaxisTargetObjectFile::IsGlobalInSmallSection(
...
IsGlobalInSmallSectionImpl(const GlobalObject *GO,
  ...
  const MaxisSubtarget &Subtarget =
      *static_cast<const MaxisTargetMachine &>(TM).getSubtargetImpl();
...
MCSection *MaxisTargetObjectFile::SelectSectionForGlobal(
...
bool MaxisTargetObjectFile::IsConstantInSmallSection(
    const DataLayout &DL, const Constant *CN, const TargetMachine &TM) const {
  return (static_cast<const MaxisTargetMachine &>(TM)
              .getSubtargetImpl()
              ->useSmallSection() &&
...
/// Return true if this constant should be placed into small data section.
MCSection *MaxisTargetObjectFile::getSectionForConstant(const DataLayout &DL,
                                                       SectionKind Kind,
                                                       const Constant *C,
...
const MCExpr *
MaxisTargetObjectFile::getDebugThreadLocalSymbol(const MCSymbol *Sym) const {
  const MCExpr *Expr =
