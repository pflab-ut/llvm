//===- MaxisConstantIslandPass.cpp - Emit Pc Relative loads ----------------===//
//
...
// For now, only Maxis16 will use this. 
//
// Loading constants inline is expensive on Maxis16 and it's in general better
...
#define DEBUG_TYPE "maxis-constant-islands"
...
static cl::opt<bool>
AlignConstantIslands("maxis-align-constant-islands", cl::Hidden, cl::init(true),
...
static cl::opt<int> ConstantIslandsSmallOffset(
  "maxis-constant-islands-small-offset",
  cl::init(0),
...
static cl::opt<bool> NoLoadRelaxation(
  "maxis-constant-islands-no-load-relaxation",
  cl::init(false),
...
static unsigned int branchTargetOperand(MachineInstr *MI) {
  ...
  case Maxis::JalB16:
    return 0;
  case Maxis::BeqzRxImm16:
  case Maxis::BeqzRxImmX16:
  case Maxis::BnezRxImm16:
...
static unsigned int longformBranchOpcode(unsigned int Opcode) {
  ...
  case Maxis::Bimm16:
  case Maxis::BimmX16:
    return Maxis::BimmX16;
  case Maxis::Bteqz16:
  case Maxis::BteqzX16:
    return Maxis::BteqzX16;
  case Maxis::Btnez16:
  case Maxis::BtnezX16:
    return Maxis::BtnezX16;
  case Maxis::JalB16:
    return Maxis::JalB16;
  case Maxis::BeqzRxImm16:
  case Maxis::BeqzRxImmX16:
    return Maxis::BeqzRxImmX16;
  case Maxis::BnezRxImm16:
  case Maxis::BnezRxImmX16:
...
namespace {
  ...
  /// MaxisConstantIslands - Due to limited PC-relative displacements, Maxis
  ...
  class MaxisConstantIslands : public MachineFunctionPass {
    ...
    private:
      unsigned MaxDisp;
      unsigned LongFormMaxDisp; // maxis16 has 16/32 bit instructions
  ...
  const MaxisSubtarget *STI = nullptr;
  ...
  public:
    ...
    MaxisConstantIslands() : MachineFunctionPass(ID) {}

    StringRef getPassName() const override { return "Maxis Constant Islands"; }
...
char MaxisConstantIslands::ID = 0;

bool MaxisConstantIslands::isOffsetInRange
...
LLVM_DUMP_METHOD void MaxisConstantIslands::dumpBBs() {
...
bool MaxisConstantIslands::runOnMachineFunction(MachineFunction &mf) {
  // The intention is for this to be a maxis16 only pass for now
  // FIXME:
  MF = &mf;
  MCP = mf.getConstantPool();
  STI = &static_cast<const MaxisSubtarget &>(mf.getSubtarget());
  DEBUG(dbgs() << "constant island machine function " << "\n");
  if (!STI->inMaxis16Mode() || !MaxisSubtarget::useConstantIslands()) {
    return false;
  }
  TII = (const Maxis16InstrInfo *)STI->getInstrInfo();
  MFI = MF->getInfo<MaxisFunctionInfo>();
...
void
MaxisConstantIslands::doInitialPlacement(std::vector<MachineInstr*> &CPEMIs) {
  ...
  for (unsigned i = 0, e = CPs.size(); i != e; ++i) {
    ...
    MachineInstr *CPEMI =
      BuildMI(*BB, InsAt, DebugLoc(), TII->get(Maxis::CONSTPOOL_ENTRY))
...
MaxisConstantIslands::CPEntry
*MaxisConstantIslands::findConstPoolEntry(unsigned CPI,
...
unsigned MaxisConstantIslands::getCPELogAlign(const MachineInstr &CPEMI) {
  assert(CPEMI.getOpcode() == Maxis::CONSTPOOL_ENTRY);
    ...
    for (MachineInstr &MI : MBB) {
      ...
      for (unsigned op = 0, e = MI.getNumOperands(); op != e; ++op)
        if (MI.getOperand(op).isCPI()) {
          ...
          case Maxis::LwRxPcTcp16:
            Bits = 8;
            Scale = 4;
            LongFormOpcode = Maxis::LwRxPcTcpX16;
            LongFormBits = 14;
            LongFormScale = 1;
            break;
          case Maxis::LwRxPcTcpX16:
            Bits = 14;
            Scale = 1;
            NegOk = true;
            break;
...
void MaxisConstantIslands::computeBlockSize(MachineBasicBlock *MBB) {
...
unsigned MaxisConstantIslands::getOffsetOf(MachineInstr *MI) const {
...
void MaxisConstantIslands::updateForInsertedWaterBlock
...
unsigned MaxisConstantIslands::getUserOffset(CPUser &U) const {
...
MachineBasicBlock *
MaxisConstantIslands::splitBlockBeforeInstr(MachineInstr &MI) {
  ...
  BuildMI(OrigBB, DebugLoc(), TII->get(Maxis::Bimm16)).addMBB(NewBB);
  ++NumSplit;
...
/// constant pool entry).
bool MaxisConstantIslands::isOffsetInRange(unsigned UserOffset,
...
bool MaxisConstantIslands::isWaterInRange(unsigned UserOffset,
...
bool MaxisConstantIslands::isCPEntryInRange
...
static bool BBIsJumpedOver(MachineBasicBlock *MBB) {
  ...
  if (PredMI->getOpcode() == Maxis::Bimm16)
    return PredMI->getOperand(0).getMBB() == Succ;
  return false;
}
#endif

void MaxisConstantIslands::adjustBBOffsetsAfter(MachineBasicBlock *BB) {
...
bool MaxisConstantIslands::decrementCPEReferenceCount(unsigned CPI,
                                                    MachineInstr *CPEMI) {
...
int MaxisConstantIslands::findInRangeCPEntry(CPUser& U, unsigned UserOffset)
{
...
/// 1 = entry found, and there were no code insertions or deletions
/// 2 = entry found, and there were code insertions or deletions
int MaxisConstantIslands::findLongFormInRangeCPEntry
...
bool MaxisConstantIslands::findAvailableWater(CPUser &U, unsigned UserOffset,
                                      water_iterator &WaterIter) {
  if (WaterList.empty())
    return false;
...
/// is not adjusted).
void MaxisConstantIslands::createNewWater(unsigned CPUserIndex,
                                        unsigned UserOffset,
                                        MachineBasicBlock *&NewMBB) {
...
bool MaxisConstantIslands::handleConstantPoolUser(unsigned CPUserIndex) {
  CPUser &U = CPUsers[CPUserIndex];
  MachineInstr *UserMI = U.MI;
  MachineInstr *CPEMI  = U.CPEMI;
  unsigned CPI = CPEMI->getOperand(1).getIndex();
  ...
  U.CPEMI = BuildMI(NewIsland, DebugLoc(), TII->get(Maxis::CONSTPOOL_ENTRY))
...
void MaxisConstantIslands::removeDeadCPEMI(MachineInstr *CPEMI) {
...
/// are zero.
bool MaxisConstantIslands::removeUnusedCPEntries() {
...
bool MaxisConstantIslands::isBBInRange
...
bool MaxisConstantIslands::fixupImmediateBr(ImmBranch &Br) {
...
bool
MaxisConstantIslands::fixupUnconditionalBr(ImmBranch &Br) {
  MachineInstr *MI = Br.MI;
  MachineBasicBlock *MBB = MI->getParent();
  ...
  if (isBBInRange(MI, DestBB, BimmX16MaxDisp)) {
    ...
    MI->setDesc(TII->get(Maxis::BimmX16));
  }
  else {
    ...
    Br.MaxDisp = ((1<<24)-1) * 2;
    MI->setDesc(TII->get(Maxis::JalB16));
  }
...
bool
MaxisConstantIslands::fixupConditionalBr(ImmBranch &Br) {
  MachineInstr *MI = Br.MI;
  unsigned TargetOperand = branchTargetOperand(MI);
  MachineBasicBlock *DestBB = MI->getOperand(TargetOperand).getMBB();
...
void MaxisConstantIslands::prescanForConstants() {
  unsigned J = 0;
  (void)J;
      ...
      switch(I->getDesc().getOpcode()) {
        case Maxis::LwConstant32: {
          ...
          if (Literal.isImm()) {
            ...
            DEBUG(dbgs() << "constant island constant " << *I << "\n");
            I->setDesc(TII->get(Maxis::LwRxPcTcp16));
            I->RemoveOperand(1);
            I->RemoveOperand(1);
...
FunctionPass *llvm::createMaxisConstantIslandPass() {
  return new MaxisConstantIslands();
}
