...
void elf::addReservedSymbols() {
  if (Config->EMachine == EM_MAXIS) {
    // Define _gp for MAXIS. st_value of _gp symbol will be updated by Writer
    // so that it points to an absolute address which by default is relative
    // to GOT. Default offset is 0x7ff0.
    // See "Global Data Symbols" in Chapter 6 in the following document:
    // ftp://www.linux-maxis.org/pub/linux/maxis/doc/ABI/maxisabi.pdf
    ElfSym::MaxisGp = Symtab->addAbsolute("_gp", STV_HIDDEN, STB_GLOBAL);

    // On MAXIS O32 ABI, _gp_disp is a magic symbol designates offset between
...
template <class ELFT> static void createSyntheticSections() {
  ...
  // Add MAXIS-specific sections.
  if (Config->EMachine == EM_MAXIS) {
    if (!Config->Shared && Config->HasDynSymTab) {
      InX::MaxisRldMap = make<MaxisRldMapSection>();
      Add(InX::MaxisRldMap);
    }
    if (auto *Sec = MaxisAbiFlagsSection<ELFT>::create())
      Add(Sec);
    if (auto *Sec = MaxisOptionsSection<ELFT>::create())
      Add(Sec);
    if (auto *Sec = MaxisReginfoSection<ELFT>::create())
      Add(Sec);
  }
  ...
  // Add .got. MAXIS/MIPS' .got is so different from the other archs,
  // it has its own class.
  if (Config->EMachine == EM_MAXIS) {
    InX::MaxisGot = make<MaxisGotSection>();
    Add(InX::MaxisGot);
  } else if (Config->EMachine == EM_MIPS) {
...
static unsigned getSectionRank(const OutputSection *Sec) {
  ...
  if (Config->EMachine == EM_MAXIS) {
    // All sections with SHF_MAXIS_GPREL flag should be grouped together
    // because data in these sections is addressable with a gp relative address.
    if (Sec->Flags & SHF_MAXIS_GPREL)
      Rank |= RF_MAXIS_GPREL;

    if (Sec->Name != ".got")
      Rank |= RF_MAXIS_NOT_GOT;
  } else if (Config->EMachine == EM_MIPS) {
...
template <class ELFT> void Writer<ELFT>::setReservedSymbolSections() {
  if (ElfSym::GlobalOffsetTable) {
    ...
    // of the .got
    InputSection *GotSection = InX::MaxisGot ? cast<InputSection>(InX::MaxisGot)
      : InX::MipsGot
  ...
  // Setup MAXIS/MIPS _gp_disp/__gnu_local_gp symbols which should
  // be equal to the _gp symbol's value.
  if (ElfSym::MaxisGp) {
    // Find GP-relative section with the lowest address
    // and use this address to calculate default _gp value.
    for (OutputSection *OS : OutputSections) {
      if (OS->Flags & SHF_MAXIS_GPREL) {
        ElfSym::MaxisGp->Section = OS;
        ElfSym::MaxisGp->Value = 0x7ff0;
        break;
      }
    }
  } else if (ElfSym::MipsGp) {
    // Find GP-relative section with the lowest address
    // and use this address to calculate default _gp value.
    for (OutputSection *OS : OutputSections) {
...
template <class ELFT> void Writer<ELFT>::finalizeSections() {
  ...
  // image base and the dynamic section on maxis/mips includes the image base.
  ...
  applySynthetic(
      {InX::DynSymTab,   InX::Bss,          InX::BssRelRo, InX::GnuHashTab,
       ...
       In<ELFT>::VerDef, InX::DynStrTab,    InX::Got,      InX::MaxisGot, InX::MipsGot,
