...
//===----------------------------------------------------------------------===//
// MAXIS ABI Implementation.  This works for both little-endian and
// big-endian variants.
//===----------------------------------------------------------------------===//

namespace {
class MaxisABIInfo : public ABIInfo {
  bool IsO32;
...
public:
  MaxisABIInfo(CodeGenTypes &CGT, bool _IsO32) :
...
class MAXISTargetCodeGenInfo : public TargetCodeGenInfo {
  unsigned SizeOfUnwindException;
public:
  MAXISTargetCodeGenInfo(CodeGenTypes &CGT, bool IsO32)
    : TargetCodeGenInfo(new MaxisABIInfo(CGT, IsO32)),
  ...
  void setTargetAttributes(const Decl *D, llvm::GlobalValue *GV,
    ...
    if (FD->hasAttr<MaxisLongCallAttr>())
      Fn->addFnAttr("long-call");
    else if (FD->hasAttr<MaxisShortCallAttr>())
    ...
    if (FD->hasAttr<Maxis16Attr>()) {
      Fn->addFnAttr("maxis16");
    }
    else if (FD->hasAttr<NoMaxis16Attr>()) {
      Fn->addFnAttr("nomaxis16");
    }

    if (FD->hasAttr<MicroMaxisAttr>())
      Fn->addFnAttr("micromaxis");
    else if (FD->hasAttr<NoMicroMaxisAttr>())
      Fn->addFnAttr("nomicromaxis");

    const MaxisInterruptAttr *Attr = FD->getAttr<MaxisInterruptAttr>();
    if (!Attr)
      return;

    const char *Kind;
    switch (Attr->getInterrupt()) {
    case MaxisInterruptAttr::eic:     Kind = "eic"; break;
    case MaxisInterruptAttr::sw0:     Kind = "sw0"; break;
    case MaxisInterruptAttr::sw1:     Kind = "sw1"; break;
    case MaxisInterruptAttr::hw0:     Kind = "hw0"; break;
    case MaxisInterruptAttr::hw1:     Kind = "hw1"; break;
    case MaxisInterruptAttr::hw2:     Kind = "hw2"; break;
    case MaxisInterruptAttr::hw3:     Kind = "hw3"; break;
    case MaxisInterruptAttr::hw4:     Kind = "hw4"; break;
    case MaxisInterruptAttr::hw5:     Kind = "hw5"; break;
    }
...
void MaxisABIInfo::CoerceToIntArgs(
...
// a register.
llvm::Type* MaxisABIInfo::HandleAggregates(QualType Ty, uint64_t TySize) const {
...
llvm::Type *MaxisABIInfo::getPaddingType(uint64_t OrigOffset,
...
ABIArgInfo
MaxisABIInfo::classifyArgumentType(QualType Ty, uint64_t &Offset) const {
  Ty = useFirstFieldIfTransparentUnion(Ty);
...
llvm::Type*
MaxisABIInfo::returnAggregateInRegs(QualType RetTy, uint64_t Size) const {
...
ABIArgInfo MaxisABIInfo::classifyReturnType(QualType RetTy) const {
  uint64_t Size = getContext().getTypeSize(RetTy);
...
void MaxisABIInfo::computeInfo(CGFunctionInfo &FI) const {
...
Address MaxisABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
...
bool MaxisABIInfo::shouldSignExtUnsignedType(QualType Ty) const {
  int TySize = getContext().getTypeSize(Ty);

  // MAXIS64 ABI requires unsigned 32 bit integers to be sign extended.
...
bool
MAXISTargetCodeGenInfo::initDwarfEHRegSizeTable(CodeGen::CodeGenFunction &CGF,
                                               llvm::Value *Address) const {
  // This information comes from gcc's implementation, which seems to
  // as canonical as it gets.

  // Everything on MAXIS is 4 bytes.  Double-precision FP registers
...
const TargetCodeGenInfo &CodeGenModule::getTargetCodeGenInfo() {
  ...
  case llvm::Triple::maxisel:
    if (Triple.getOS() == llvm::Triple::NaCl)
      return SetCGInfo(new PNaClTargetCodeGenInfo(Types));
    return SetCGInfo(new MAXISTargetCodeGenInfo(Types, true));

  case llvm::Triple::maxis64:
  case llvm::Triple::maxis64el:
    return SetCGInfo(new MAXISTargetCodeGenInfo(Types, false));
